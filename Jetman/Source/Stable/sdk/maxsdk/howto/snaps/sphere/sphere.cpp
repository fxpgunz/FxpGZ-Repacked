...
TSTR SphereSnap::iconName(int index) const
{
...
SphereSnap::SphereSnap()
{
	...
	iconNames[CENTER_SUB]= _T("SnapTools/MaxSDK/Sphere/SphereCenter");
	iconNames[QUAD_SUB]= _T("SnapTools/MaxSDK/Sphere/SphereQuad");
};
...
{	
	...
	if(	GetActive(QUAD_SUB))
	{
		...
		float radius;
		IParamBlock2* sphereParams = ((GenSphere *)pobj)->GetParamBlockByID(SPHERE_PARAMBLOCK_ID);
		DbgAssert(sphereParams);
		if (sphereParams == nullptr)
			return;
		sphereParams->GetValue(SPHERE_RADIUS,t,radius,FOREVER);
