...
amg_output_struct pixel_main(AMG_PixelIn fs_in)
{
   ...
   sum += (tex2D(sampler_texturen2, float2(amg_uvw.x, amg_uvw.y - 4.0*blurSize)) * 0.05).rgb;
   sum += (tex2D(sampler_texturen2, float2(amg_uvw.x, amg_uvw.y - 3.0*blurSize)) * 0.09).rgb;
   sum += (tex2D(sampler_texturen2, float2(amg_uvw.x, amg_uvw.y - 2.0*blurSize)) * 0.12).rgb;
   sum += (tex2D(sampler_texturen2, float2(amg_uvw.x, amg_uvw.y - blurSize)) * 0.15).rgb;
