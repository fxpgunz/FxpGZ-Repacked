...
BOOL LayerBoss::BossProc(HWND hDlg, UINT msg, WPARAM  wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		{		
			...
			// Update the listbox
			SendDlgItemMessage(hDlg, IDC_LAYER_LIST, LB_ADDSTRING, NULL, (LPARAM) _T("=========================" ));
			SendDlgItemMessage(hDlg, IDC_LAYER_LIST, LB_ADDSTRING, NULL, (LPARAM) _T("Listing of Scene Objects" ));
			...
			for (int i = 0; i < objectCount; i++)
			{
				...
				TSTR str2 = _T(", is on layer ");
				//Get the layer name
				TSTR str3 = aLayer->GetName();
				str1 = str1 + str2 + str3;
				
				//Add the string to the list box here
				SendDlgItemMessage(hDlg, IDC_LAYER_LIST, LB_ADDSTRING, NULL, (LPARAM) (const TCHAR*) str1  );
			}		
			// Update the listbox
			SendDlgItemMessage(hDlg, IDC_LAYER_LIST, LB_ADDSTRING, NULL, (LPARAM) _T("=========================" ));
			SendDlgItemMessage(hDlg, IDC_LAYER_LIST, LB_ADDSTRING, NULL, (LPARAM) _T("Listing of Scene Layers" ));
			...
			for (int i = 0; i < layerCount; i++)
			{
				...
				//Get the layer name
				const TCHAR* layerName = hLayer->getName();
				...
				for (int i = 0; i < objs.Count(); i++)
				{
					...
					str3 += str4;
					SendDlgItemMessage(hDlg, IDC_LAYER_LIST, LB_ADDSTRING, NULL, (LPARAM) (const TCHAR*) str3  );
				}
			} //End for loop
			return TRUE;
		}
