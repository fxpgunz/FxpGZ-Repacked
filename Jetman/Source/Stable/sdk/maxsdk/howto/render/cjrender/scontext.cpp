...
{
	SCDebugPrint(_T("SContext::SContext\n"));
...
{
	SCDebugPrint(_T("SContext::CalcNormals\n"));
...
{
	SCDebugPrint(_T("SContext::SetBary\n"));
...
{
	SCDebugPrint(_T("SContext::ProjType\n"));
...
{
	SCDebugPrint(_T("SContext::SetInstance\n"));
	pInst = instance;
...
{
	SCDebugPrint(_T("SContext::SetFaceNum\n"));
	faceNum = f;
}

void SContext::SetScreenPos(IPoint2 pt)
{
	SCDebugPrint(_T("SContext::SetScreenPos\n"));
	screenPos = pt;
}

void SContext::SetMtlNum(int mNum)
{
	SCDebugPrint(_T("SContext::SetMtlNum\n"));
...
{
	SCDebugPrint(_T("SContext::SetHitPos\n"));
	hitPos = p;
}

void SContext::SetViewDir(Point3 v)
{
	SCDebugPrint(_T("SContext::SetViewDir\n"));
	viewDir = v;
}
...
{
	SCDebugPrint(_T("SContext::CurTime\n"));
...
int SContext::NodeID()
{
	SCDebugPrint(_T("SContext::NodeID\n"));
	return pInst->nodeID;
}
...
{
	SCDebugPrint(_T("SContext::BarycentricCoords\n"));
	return baryCoord;
}
...
{
	SCDebugPrint(_T("SContext::FaceNumber\n"));
	return faceNum;
}
...
Point3 SContext::Normal()
{
	...
	SCDebugPrint(_T("SContext::Normal\n"));
...
{
	// The face normals are already in camera space
	SCDebugPrint(_T("SContext::GNormal\n"));
...
LightDesc *SContext::Light(int lightNum)
{
	...
	SCDebugPrint(_T("SContext::Light\n"));
	return rLight->pDesc;
}
...
{
	SCDebugPrint(_T("SContext::ReflectVector\n"));
	Point3 N = Normal();
...
{
	SCDebugPrint(_T("SContext::RefractVector\n"));
	Point3 N = Normal();
	float VN,nur,k;
...
{
	SCDebugPrint(_T("SContext::CamPos\n"));
	return cameraPos;
}
...
{
	SCDebugPrint(_T("SContext::ScreenPos\n"));
	return screenPos;
}
...
{
	SCDebugPrint(_T("SContext::GetBGColor\n"));
...
Point3 SContext::PointTo(const class Point3 &p, RefFrame ito)
{
	...
	SCDebugPrint(_T("SContext::PointTo\n"));
...
Point3 SContext::PointFrom(const class Point3 &p, RefFrame ito)
{
	...
	SCDebugPrint(_T("SContext::PointFrom\n"));
...
Point3 SContext::VectorTo(const class Point3 &p, RefFrame ito)
{
	Point3 pt;
	Matrix3 camToObj = Inverse(pInst->objToWorld * renderer->view.affineTM);
	SCDebugPrint(_T("SContext::VectorTo\n"));
...
Point3 SContext::VectorFrom(const class Point3 &p, RefFrame ito)
{
	...
	SCDebugPrint(_T("SContext::VectorFrom\n"));
	switch (ito) {
...
void SContext::ScreenUV(class Point2 &uv,class Point2 &duv)
{
	...
	SCDebugPrint(_T("SContext::ScreenUV\n"));
...
{
	SCDebugPrint(_T("SContext::ObjectBox\n"));
	return pInst->obBox;
}
...
{
	SCDebugPrint(_T("SContext::PObjRelBox\n"));
...
{
	SCDebugPrint(_T("SContext::DPObjRelBox\n"));
	Box3 b = ObjectBox(); 
	Point3 d = DPObj();
...
{
	SCDebugPrint(_T("SContext::DPObj\n"));
...
{
	SCDebugPrint(_T("SContext::DPdUVW\n"));
...
{
	SCDebugPrint(_T("SContext::Curve\n"));
...
{
	SCDebugPrint(_T("BGContext::SetScreenPos\n"));
	iscr = IPoint2(x,y);
...
Point3 BGContext::BGGetPoint(float z) {
	SCDebugPrint(_T("BGContext::BGGetPoint\n"));
...
void BGContext::ScreenUV(Point2& uv, Point2 &duv) {
	SCDebugPrint(_T("BGContext::ScreenUV\n"));
	Point2 p;
...
Point3 BGContext::PointTo(const Point3& p, RefFrame ito) {
	SCDebugPrint(_T("BGContext::PointTo\n"));
...
Point3 BGContext::PointFrom(const Point3& p, RefFrame ifrom){ 
	SCDebugPrint(_T("BGContext::PointFrom\n"));
...
Point3 BGContext::VectorTo(const Point3& p, RefFrame ito) {
	SCDebugPrint(_T("BGContext::VectorTo\n"));
	switch (ito) {
...
Point3 BGContext::VectorFrom(const Point3& p, RefFrame ifrom){ 
	SCDebugPrint(_T("BGContext::VectorFrom\n"));
	switch (ifrom) {
