...
static Class_ID ConvertBSTRtoClassID(BSTR id)
{
	...
	TSTR sub2 = cid.Substr(sub+1,len-(sub+1));
	ulong aa = _tcstoul(sub1.data(),NULL,16);
	ulong bb = _tcstoul(sub2.data(),NULL,16);
...
void StackBoss::DeleteThis() {
	delete this;
}

DWORD_PTR StackBoss::Control( DWORD parameter ) {
	return 0;
}
...
{
	TCHAR path[256];
	WStr  strIncludedEntity (L"included");
	WStr  strExcludedEntity (L"excluded");
	WStr  strGlobal (L"Global");
	LONG value;
	HRESULT hr;
	...
	BSTR tagName = strGlobal.ToBSTR();
	hr = pXMLDoc->getElementsByTagName(tagName,&pIDOMNodeList);
	...
	tagName = strIncludedEntity.ToBSTR();
	hr = pXMLDoc->getElementsByTagName(tagName,&pIDOMNodeList);
	::SysFreeString(tagName);
	tagName = NULL;
	if(FAILED(hr))
	{
		tagName = strExcludedEntity.ToBSTR();
		hr = pXMLDoc->getElementsByTagName(tagName,&pIDOMNodeList);
		::SysFreeString(tagName);
		tagName = NULL;
		if(FAILED(hr))
...
void StackBoss::AddStackBossModifier(INode * node, Modifier * mod, ModContext * mContext)
{
	...
	stackBossMod->SetName(_T("Stack Boss"));
