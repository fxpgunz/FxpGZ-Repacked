...
Point3 AsciiImp::GetPoint3()
{
	...
	token = GetToken();
	p.x = (float)_tstof(token);
	token = GetToken();
	p.y = (float)_tstof(token);
	token = GetToken();
	p.z = (float)_tstof(token);
...
DWORD AsciiImp::GetSmoothingGroups(TCHAR* smStr)
{
	...
	token = _tcstok(smStr, _T(","));
	while(token) 
	{
		smGroup = smGroup + (1<<(_ttoi(token)-1));
