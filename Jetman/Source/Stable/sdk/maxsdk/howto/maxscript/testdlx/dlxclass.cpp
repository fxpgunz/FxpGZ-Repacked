...
ListValueClass::apply(Value** arg_list, int count,  CallContext* cc)
{
	...
	two_typed_value_locals_tls(ListValue* result, Value* head);
...
ListValue::map(node_map& m)
{
	ListValue*	t;
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	two_value_locals_tls(result, arg_save);
...
SetValueClass::apply(Value** arg_list, int count,  CallContext* cc)
{
	// set v1 v2 ... vn
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	two_typed_value_locals_tls(SetValue* result, Value* head);
...
SetValue::intersection_vf(Value** arg_list, int count)
{
	...
	MAXScript_TLS* _tls = (MAXScript_TLS*)TlsGetValue(thread_locals_index);
	one_typed_value_local_tls(SetValue* result);
	ListValue* t;
	vl.result = new SetValue();
