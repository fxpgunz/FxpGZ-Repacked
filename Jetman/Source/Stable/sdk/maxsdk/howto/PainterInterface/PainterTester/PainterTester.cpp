...
void PainterTester::CreateCylinder(BOOL isMirror, Point3 pos, Point3 normal,float frad, float fheight)
	{
	SimpleObject2 *obj = (SimpleObject2*)GetCOREInterface()->CreateInstance(GEOMOBJECT_CLASS_ID,Class_ID(CYLINDER_CLASS_ID,0));
	assert(obj);
	IParamBlock2 *pb2 = obj->GetParamBlockByID(CYLINDER_PARAMBLOCK_ID);
	DbgAssert(pb2);
	if (pb2 == nullptr)
		return;

	// Set the value of radius, height and segs.
	pb2->SetValue(CYLINDER_RADIUS,TimeValue(0),frad);
	pb2->SetValue(CYLINDER_HEIGHT,TimeValue(0),fheight);
	pb2->SetValue(CYLINDER_SEGMENTS,TimeValue(0),1);
...
//	ip->MakeNameUnique(name);
	...
	if (isMirror)
		mirrorNodeList.Append(1,&pb2,100);
	else nodeList.Append(1,&pb2,100);
	...
	if ((radius) || (str))
		...
		for (int i =0; i < ct; i++)
			...
			if (i < nodeList.Count())
				{
				if (radius)
					nodeList[i]->SetValue(CYLINDER_RADIUS,TimeValue(0),radius[i]);
				if (str)
					nodeList[i]->SetValue(CYLINDER_HEIGHT,TimeValue(0),str[i]);

				}
			if (i < mirrorNodeList.Count())
				{
				if (radius)
					mirrorNodeList[i]->SetValue(CYLINDER_RADIUS,TimeValue(0),radius[i]);
				if (str)
					mirrorNodeList[i]->SetValue(CYLINDER_HEIGHT,TimeValue(0),str[i]);
