...
#define CA_COUNT 4
static  TCHAR * ca_name[CA_COUNT] = {_T("Ca 1"),_T("Ca 2"),_T("Ca 3"),_T("Mat 1")};


ClassDesc2* GetDynPBlockDesc() {return &DynPBlockDesc;}


static INT_PTR CALLBACK DynPBlockDlgProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
...
int DynPBlock::CheckCCIsLoaded(ICustAttribContainer * cc,int which,BOOL Mat)
{
	...
	for(int i=0;i<cc->GetNumCustAttribs();i++)
	{
		...
		if(!Mat){
			...
			if(!_tcscmp(pd->cd->ClassName(),buf))
			{
				return i;
			}
		}
		else
		{
			TCHAR  buf[50];
			ListBox_GetText(hWndList, which, &buf);
			const TCHAR *test = pd->cd->ClassName();
			if(!_tcscmp(pd->cd->ClassName(),buf))
			{
				return i;
			}
...
//  Is our CA already installed ?
	...
	{
		MessageBox(hPanel,_T("CA Already Present, Not Loading"), strWarning, NULL);
		return;
	}
...
void DynPBlock::Remove()
{
	...
	if(cc!=NULL)
	{
		...
		if(index >= 0) {
			...
			MessageBox(hPanel,_T("Nothing to Remove"), strWarning, NULL);
...
void DynPBlock::AddMaterialCA(BOOL Insert)
{
	...
	{
		MessageBox(hPanel,_T("CC Already Present, Not Loading"), strWarning, NULL);
		return;
	}
	theHold.Begin(); 
