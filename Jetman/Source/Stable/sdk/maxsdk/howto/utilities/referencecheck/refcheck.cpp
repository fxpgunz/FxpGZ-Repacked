...
class NotifyMgr : public ReferenceTarget {
   private:
      int nNumRefs;
      RefResult (*pNotifyFunc)(const Interval& changeInt,
...
public:
      ...
      void SetNotifyFunc(RefResult (*func)(const Interval& changeInt,
...
void NotifyMgr::SetNotifyFunc(RefResult (*func)(const Interval& changeInt,
   RefTargetHandle hTarget, PartID& partID,RefMessage message, 
      DWORD_PTR pData), DWORD_PTR pData) {
   pPrivateData = pData;
   pNotifyFunc = func;
...
RefResult ProcessRefMessage(const Interval& changeInt, RefTargetHandle hTarget,
