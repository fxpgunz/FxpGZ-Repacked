...
public:
	...
	ULONG GetRequirements( int /*i*/){ return MTLREQ_PHONG; }
...
private:
	...
	ISpinnerControl* brSpin;
	ISpinnerControl* trSpin;
...
public:
	...
	void SetRollup(HWND val) { hRollup = val; }
	...
	void UpdateDialog( ParamID /*id*/){ ReloadDialog(); }
...
ShaderParamDlg* [!output CLASS_NAME]::CreateParamDialog(HWND hOldRollup, HWND hwMtlEdit, IMtlParams *imp, StdMat2* theMtl, int rollupOpen,int )
{
	...
	if ( hOldRollup ) {
		HWND replaced = imp->ReplaceRollupPage(
			hOldRollup,
			hInstance,
		...
		pDlg->SetRollup(replaced);
	}
	else
	{
		HWND replaced = imp->AddRollupPage(
			hInstance,
			MAKEINTRESOURCE(IDD_PANEL),
			[!output CLASS_NAME]DlgProc,
			GetString(IDS_PARAMS),
...
void [!output CLASS_NAME]::Illum(ShadeContext &sc, IllumParams &ip)
{
	...
	for (int i=0; i<sc.nLights; i++) {
		...
		Point3 L;
		Point3 norm = sc.Normal();
		if (l->Illuminate(sc,norm,lightCol,L,NL,diffCoef)) {
			if (l->ambientOnly) {
