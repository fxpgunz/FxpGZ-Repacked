...
bool MMatchEventDescManager::LoadEventXML( const string& strFileName, MZFileSystem* pFileSystem)
{
	...
	for (auto itor = rootNode->first_node(); itor; itor = itor->next_sibling()) 
	{
		...
		if (_stricmp(itor->name(), EV_EVENT) == 0)
			ParseEvent(itor);
	}
...
void MMatchEventDescManager::ParseEvent( rapidxml::xml_node<>* chrElement )
{
	MMatchEventDesc* pEventDesc = new MMatchEventDesc;
	if( 0 != pEventDesc )
	{
		pEventDesc->Set( strtoul(chrElement->first_attribute(EV_EVENTID)->value(),0,10), chrElement->first_attribute(EV_DESC)->value());

		Insert( strtoul(chrElement->first_attribute(EV_EVENTID)->value(),0,10), pEventDesc );
	}
}
