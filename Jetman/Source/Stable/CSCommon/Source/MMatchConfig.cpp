...
bool MMatchConfig::Create()
{
	...
	if (!_stricmp(szServerMode, SERVER_CONFIG_SERVERMODE_NORMAL)) m_nServerMode = MSM_USUAL;
	...
	m_bEnabledSurvivalMode = (1 != GetPrivateProfileInt("SERVER", "SURVIVALENABLE", 1, SERVER_CONFIG_FILENAME));
...
void MMatchConfig::TrimStr(const char* szSrcStr, char* outStr)
{
	...
	for (int i = 0; i < nSrcStrLen; i++)
	{
		...
		{
			strcpy_s(szInputMapName, &szSrcStr[i]);
			break;
		}
	}
	// 오른쪽 공백제거
