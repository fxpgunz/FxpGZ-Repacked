						...
						const bool bIsTicketChannel, const DWORD dwTicketItemID, const bool bIsUseTicket, const char* pszNameStrResId)
{
	...
	m_uidChannel		= uid;
	strcpy_s(m_szChannelName, pszName);
	strcpy_s(m_szRuleName, pszRuleName);
	...
	if (pszNameStrResId) {
		...
		strcpy_s(m_szChannelNameStrResId, pszNameStrResId);
	}
	...
	if (nType == MCHANNEL_TYPE_PRESET || nType == MCHANNEL_TYPE_DUELTOURNAMENT)
	{
		...
		else if (!_stricmp(m_szRuleName,"EVENT"))							m_nRuleType = MCHANNEL_RULE_EVENT;
	}
