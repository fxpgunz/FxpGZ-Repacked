...
MCommandDesc* MCommandManager::GetCommandDescByID(unsigned short nID)
{
...
#define ASMESSAGE_LENGTH	256
	...
	strcpy_s(szTemp, sizeof(szTemp),la.GetByStr(0));

	MCommandAliasMap::iterator itor = m_CommandAlias.find(string(szTemp));
	if (itor != m_CommandAlias.end())
	{
		strcpy_s(szTemp, sizeof(szTemp),(*itor).second.c_str());
	}
