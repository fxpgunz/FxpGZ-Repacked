...
void CopyClanMemberListNodeForTrans(MTD_ClanMemberListNode* pDest, MMatchObject* pSrcObject)
{
	...
	strcpy_s(pDest->szName, pSrcObject->GetCharInfo()->m_szName);
...
void MMatchServer::UpdateCharClanInfo(MMatchObject* pObject, const int nCLID, const char* szClanName, const MMatchClanGrade nGrade)
{
	...
	// object의 정보 변경
	strcpy_s(pObject->GetCharInfo()->m_ClanInfo.m_szClanName, szClanName);
	...
	if (nGrade == MCG_MASTER)
	{
		...
		if (pClan)
			strcpy_s(pClan->GetClanInfoEx()->szMaster, pObject->GetCharInfo()->m_szName);
	}
	...
	strcpy_s(pClanInfo->szClanName, szClanName);
					...
					char** szSponsorNames)
{
	...
	int nRet = ValidateCreateClan(szClanName, pMasterObject, NULL);//pSponsorObjects);
...
void MMatchServer::OnClanRequestAgreedCreateClan(const MUID& uidPlayer, const char* szClanName, char** szSponsorNames)
{
	...
	//if (CLAN_SPONSORS_COUNT == 4)
	{
		...
		pNewJob->Input(szClanName,
			...
			MUID(0, 0),//pSponsorObjects[0]->GetUID(),
			MUID(0, 0),//pSponsorObjects[1]->GetUID(),
			MUID(0, 0),//pSponsorObjects[2]->GetUID(),
			MUID(0, 0)//pSponsorObjects[3]->GetUID()
		);
...
void MMatchServer::OnClanRequestMsg(const MUID& uidSender, const char* szMsg)
{
	...
	char szTransMsg[512];
	strcpy_s(szSenderName, pSenderObject->GetCharInfo()->m_szName);
	strcpy_s(szTransMsg, szMsg);
...
{
	strcpy_s(pDest->szClanName, pClan->GetName());
	strcpy_s(pDest->szMaster, pClan->GetClanInfoEx()->szMaster);
...
void MMatchServer::StandbyClanList(const MUID& uidPlayer, int nClanListStartIndex, bool bCacheUpdate)
{
	...
	for (int i = 0; i < nRealCount; i++)
	{
		...
		if (pLadderGroup->GetPlayerCount() > 0)
		{
			...
			{
				strcpy_s(pNode->szClanName, pMember->GetCharInfo()->m_ClanInfo.m_szClanName);
			}

			pNode->nPlayers = static_cast<short>(pLadderGroup->GetPlayerCount());
