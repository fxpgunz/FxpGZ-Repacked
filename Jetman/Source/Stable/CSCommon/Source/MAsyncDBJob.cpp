...
bool MAsyncDBJob_InsertGameLog::Input(const unsigned int nMasterCID, const char* szMap, const char* szGameType)
{
	...
	strcpy_s(m_szMap, szMap);
	strcpy_s(m_szGameType, szGameType);
...
{
	strcpy_s(m_szClanName, szClanName);
	m_nMasterCID = nMasterCID;
...
{
	m_uidAdmin = uidAdmin;
	strcpy_s(m_szTarMember, szTarMember);
	m_nCLID = nCLID;
											...
											const int nElapsedPlayerTime )
{
	...
	strcpy_s( m_szStageName, pszStageName );
