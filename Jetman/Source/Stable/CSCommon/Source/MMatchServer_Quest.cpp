...
void MMatchServer::OnQuestRequestDead(const MUID& uidVictim)
{
	...
	else if (MGetGameTypeMgr()->IsNewQuestDerived(pStage->GetStageSetting()->GetGameType())) {
		IMatchRuleNewQuest* pRule = dynamic_cast<IMatchRuleNewQuest*>(pStage->GetRule());
		pRule->OnRequestPlayerDead((MUID&)uidVictim);
		pVictim->OnDead();
	}
...
{
#ifdef _DEBUG
//	if (MGetServerConfig()->GetServerMode() != MSM_CLAN) return;
//
