...
void MCommandProfiler::PrintCmdItems(FILE* fp)
{
	...
	char text[512];
	for (unsigned short i = 0; i < MAX_COMMANDPROFILER_CMD_COUNT; ++i)
	{
		...
		if (m_pCM)
		{
			...
			if (pDesc)
				strcpy_s(szDesc,sizeof(szDesc), pDesc->GetName());
		}
		...
		if (m_Items[i].nRecvCount != 0)
		{
			sprintf_s(text,sizeof(text), "\t\t ||  Recv  || %6d(%4.1f%%)  |     %4.1f    |    %6.1f(%4.1f%%)  |   %6u(%4.1f%%) |  %6u   |  %6u   |\n",
