...
private:
	...
	void ParseSpecialScenario(rapidxml::xml_node<>* element);
	void ParseStandardScenario(rapidxml::xml_node<>* element);
	void ParseNPCSetArray(rapidxml::xml_node<>* element, vector<int>& vec);
	void ParseKeyNPCArray(rapidxml::xml_node<>* element, vector<int>& vec);
	void ParseSectorXpBpArray(rapidxml::xml_node<>* element, vector<int>& vec);
	void ParseJaco(rapidxml::xml_node<>* element, MQuestScenarioInfoMaps* pMap);
	void ParseRewardItem(rapidxml::xml_node<>* element, MQuestScenarioInfo* pScenarioInfo);
	void ParseSacriItem(rapidxml::xml_node<>* element, MQuestScenarioInfo* pScenarioInfo);
	void ParseMap(rapidxml::xml_node<>* element, MQuestScenarioInfo* pScenarioInfo);
	int CalcStandardScenarioID(int nMapsetID, int nQL);
public:
	MQuestScenarioCatalogue();											///< 생성자
	~MQuestScenarioCatalogue();											///< 소멸자

	bool ReadXml(const char* szFileName, MZFileSystem* pFileSystem = nullptr);		///< xml로부터 npc정보를 읽는다.
