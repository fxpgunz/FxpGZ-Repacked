...
class MCommandDesc{
protected:
	unsigned short			m_nID;					///< Command ID
...
public:
	...
	MCommandDesc(unsigned short nID, const char* szName, const char* szDescription, int nFlag);
	...
	/// 아이디 얻기
	unsigned short GetID(void) const { return m_nID; }
	/// 이름 얻기
	...
	MCommandParameterType GetParameterType(unsigned short i) const
	{
...
public:
	...
	MCommand::MCommand(unsigned short nID, MUID Sender, MUID Receiver, MCommandManager* pCommandManager);
	virtual ~MCommand(void);

	/// MCommandDesc으로 ID 지정
	void SetID(const MCommandDesc* pCommandDesc);
	/// ID 지정
	void MCommand::SetID(unsigned short nID, MCommandManager* pCommandManager);
	/// ID 얻기
	unsigned short GetID(void) const { return m_pCommandDesc->GetID(); }
	/// 설명 얻기

	bool IsFlag(int nFlag) { return m_pCommandDesc->IsFlag(nFlag); }
