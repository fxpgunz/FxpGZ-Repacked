...
public:
	...
	bool Create(const MUID& uid, const char* pszName, bool bPrivate, const char* pszPassword, bool bIsAllowNullChannel,vector<int> stageMods,
	...
	void SetPassword(const char* pszPassword)	{ strcpy_s(m_szStagePassword, sizeof(m_szStagePassword),pszPassword); }
	...
	void SetFirstMasterName(char* pszName)	{ strcpy_s(m_szFirstMasterName, sizeof(m_szFirstMasterName),pszName); }
	...
	bool CheckIfUIDCacheEmpty() { return m_ObjUIDCaches.empty(); }
...
public:
	...
	void SetDuelTournamentMatchTimeStamp(char* szTimeStamp)				{ strcpy_s(m_nDTStageInfo.szTimeStamp,sizeof(m_nDTStageInfo.szTimeStamp), szTimeStamp); }
