...
ZRuleSoloClassic::ZRuleSoloClassic(ZMatch* pMatch) : ZRule(pMatch)
{
...
ZRuleTeamClassic::ZRuleTeamClassic(ZMatch* pMatch) : ZRule(pMatch)
{
...
void ZRuleTeamCTF::AssignFlagEffect(MUID& uidOwner, int nTeam)
{
	...
	itor != ZGetGame()->m_CharacterManager.end(); ++itor)
	{
		...
		if(pCharacter)
		{
			...
			ZGetEffectManager()->AddFlagEffect(pCharacter);
		}
	}
	...
	if (pFlagChar)
	{
		...
		ZGetEffectManager()->AddFlagEffect(pFlagChar);
		if (nTeam == MMT_BLUE)
		{
