...
{
	//return false;
	m_friendly = new MBitmapR2();
	m_enemy = new MBitmapR2();
	m_friendly->Create("friendlyplayer", RGetDevice(),"crosshair04.png");
	m_enemy->Create("enemyplayer",RGetDevice(),"crosshair04_pick.png");
	//m_pBitmap = (MBitmapR2*)MBitmapManager::Get("radar01.bmp");
	//m_friendly = (MBitmapR2*)MBitmapManager::Get("Blitz_Barricade_Red.png");
	//m_enemy = (MBitmapR2*)MBitmapManager::Get("Blitz_Barricade_Blue.png");
	if (/*!m_pBitmap || */!m_friendly || !m_enemy)
	{
		SAFE_DELETE(m_pBitmap);
		SAFE_DELETE(m_friendly);
		SAFE_DELETE(m_enemy);
		return false;;
	}
	/*
...
void ZRadar::OnDraw(MDrawContext* pDC)
{
	...
	for (auto itor  = ZGetCharacterManager()->begin(); itor != ZGetCharacterManager()->end(); ++itor)
	{
		...
		if (player == nullptr || player->IsDie() || player->IsAdminName())
			continue;

		if (player->GetTeamID() == ZGetGame()->m_pMyCharacter->GetTeamID())
		{
			rvector pos = player->GetCenterPos();
			Normalize(pos);
			pDC->SetEffect(MDE_ADD);
			pDC->SetBitmap(m_friendly);
			pDC->Draw(50, 50, m_friendly->GetWidth(), m_friendly->GetHeight());
		}
		else
		{
