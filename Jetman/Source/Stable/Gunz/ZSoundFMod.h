...
constexpr int SOUND_DRIVER_NAME_MAX_LENGTH = 32;
...
public:
	...
	///Custom: mp3
	void pause();
	void		GetTrackLength(unsigned int& trackLength);
	const char* GetTrackLength();
	const char* GetTrackTime();
	void GetTrackTime(unsigned int& trackTime);
	void FastForward(const unsigned int value);
	void Rewind(const unsigned int value);
	void SetTrackTime(const unsigned int& value);
	bool IsPlaying();
	// ETC
