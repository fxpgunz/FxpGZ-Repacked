	...
	case MC_MATCH_DUEL_QUEUEINFO:
		{
			...
			if (QInfo.m_bIsRoundEnd)
			{
				...
				{
					ZCharacter* cha = (ZCharacter*) ZGetCharacterManager()->Find(QInfo.m_uidChampion);
					if (cha != NULL)
						cha->Revival();
					cha = (ZCharacter*) ZGetCharacterManager()->Find(QInfo.m_uidChallenger);
					if (cha != NULL)
						cha->Revival();
			...
			else
			{
				...
				{
					ZCharacter* cha = (ZCharacter*) ZGetCharacterManager()->Find(QInfo.m_WaitQueue[i]);
					if (cha != NULL)
					{
