...
void CCommandLogView::AddCommand(unsigned long int nGlobalClock, CCommandType t, MCommand* pCmd)
{
	...
	if(t==CCT_LOCAL) strcpy_s(szType,sizeof(szType), "Local");
	else if(t==CCT_SEND) strcpy_s(szType,sizeof(szType), "Send");
	else if(t==CCT_RECEIVE) strcpy_s(szType, sizeof(szType),"Receive");
