...
bool MZip::RecoveryZip(const char* zip_name)
{
	FILE* fp = nullptr;
	errno_t result = fopen_s(&fp,zip_name, "rb+");
...
void FNode::SetName(char* str)	
{
	...
	strcpy_s(m_name,sizeof(m_name),str);
	str[255] = 0;
}
...
//		ShellExecute()
		strcpy_s(temp,sizeof(temp),pNode->m_name);
		len = strlen(temp);
		temp[len-3] = 0;
		strcat(temp,"tga");
...
void FFileList::ConvertNameMRes2Zip() 
{
	...
	for(node = begin(); node != end(); ++node) {
		...
		strcpy_s(_buf_rename,sizeof(_buf_rename),pNode->m_name);
	...
	if( (hFile = _findfirst( path , &file_t )) != -1L ) {
		do{
			...
			if(file_t.attrib & _A_SUBDIR ) {
				char _path[256];
				strcpy_s(_path,sizeof(_path),file_t.name);
				strcat(_path,"/");
				strcat(_path,path);
			...
			if(_stricmp(pName,ext)==0) {
				...
				if(len > 3) {
					...
					strcpy_s(temp_name,sizeof(temp_name),file_t.name);
				}
...
bool GetFindFileListWin(const char* path,char* ext,FFileList& pList)
{
		...
		do{
			...
			if(_stricmp(pName,ext)==0) {
				...
				if(len > 1) {
					...
					strcpy_s(temp_name,sizeof(temp_name),file_t.cFileName);
				}
