...
bool MakePath(const char* pszFileName)
{
	...
	strcpy_s(szPath, sizeof(szPath),pszFileName);
	...
	if (PathIsDirectory(szPath)) {
		...
		if (CreateDirectory(szPath, NULL)) {
			...
			strcpy_s(szParent, sizeof(szParent),pszFileName);
...
bool GetMusicPath(char* path)
{
	if (path == NULL)
		return false;

	if (SUCCEEDED(SHGetSpecialFolderPath(NULL, path, CSIDL_MYMUSIC, TRUE)))
		return true;
