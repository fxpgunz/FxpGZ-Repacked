...
void _SetError(int nErrCode,const char *pErrSubStr,const char *pFileName,int nLineNum,const char *pLastModification,const char *pErrStr)
{
	...
	g_nErrCode=nErrCode;
	if(pErrSubStr!=NULL)
		strcpy_s(g_pErrSubStr,pErrSubStr);
	strcpy_s(g_pFileName,pFileName);
	g_nLineNum=nLineNum;
	strcpy_s(g_pLastModification,pLastModification);
	strcpy_s(g_pErrStr,pErrStr);
	...
	sprintf_s(g_pGetLastErrStr, "%s", reinterpret_cast<char*>(lpMsgBuf));
	LocalFree( lpMsgBuf );
#endif
}
