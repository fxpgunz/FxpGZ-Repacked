...
void DrawNavFace(LPDIRECT3DDEVICE9 pLPDIRECT3DDEVICE9, rvector* vertices, int v1, int v2, int v3)
{
	rvector v[3];
	v[0] = vertices[v1];
	v[1] = vertices[v2];
	...
	pLPDIRECT3DDEVICE9->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 1, v, sizeof(rvector));
}

void DrawNavFaceWireFrame(LPDIRECT3DDEVICE9 pLPDIRECT3DDEVICE9, rvector* vertices, int v1, int v2, int v3)
{
	rvector v[4];
	v[0] = vertices[v1];
	v[1] = vertices[v2];
	...
	pLPDIRECT3DDEVICE9->DrawPrimitiveUP(D3DPT_LINESTRIP,3,&v,sizeof(rvector));
}
...
void RNavigationMesh::Render()
{
	...
	pLPDIRECT3DDEVICE9->SetRenderState(D3DRS_TEXTUREFACTOR , 0x40FF9100);
...
/*
	for (RNodeArray::iterator itor = m_NodeArray.begin(); itor != m_NodeArray.end(); ++itor)
	{
		...
		if (pNode->bSelected) pLPDIRECT3DDEVICE9->SetRenderState(D3DRS_TEXTUREFACTOR , 0x40FFFFFF);
		else pLPDIRECT3DDEVICE9->SetRenderState(D3DRS_TEXTUREFACTOR , 0x40FF9100);
		...
		DrawNavFace(pLPDIRECT3DDEVICE9, _vertices, 0, 1, 2);
	}
*/
	RSetWBuffer(true);
	...
	for (RNodeArray::iterator itor = m_NodeArray.begin(); itor != m_NodeArray.end(); ++itor)
	{
		...
		DrawNavFaceWireFrame(pLPDIRECT3DDEVICE9, _vertices, 0, 1, 2);
...
void RNavigationMesh::RenderLinks()
{
	...
	pLPDIRECT3DDEVICE9->SetRenderState(D3DRS_ALPHABLENDENABLE, FALSE);
	pLPDIRECT3DDEVICE9->SetRenderState(D3DRS_TEXTUREFACTOR , 0xFF0033FF);
	pLPDIRECT3DDEVICE9->SetTextureStageState( 0, D3DTSS_COLORARG1, D3DTA_TFACTOR);
	pLPDIRECT3DDEVICE9->SetTextureStageState( 0, D3DTSS_COLOROP,   D3DTOP_SELECTARG1 );
	pLPDIRECT3DDEVICE9->SetTextureStageState( 0, D3DTSS_ALPHAARG1, D3DTA_TFACTOR);
	pLPDIRECT3DDEVICE9->SetTextureStageState( 0, D3DTSS_ALPHAOP,   D3DTOP_SELECTARG1 );
