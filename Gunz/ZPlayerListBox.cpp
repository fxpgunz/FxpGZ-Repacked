...
/*
void ZPlayerListBox::AddPlayer(MUID& puid, ePlayerState state, int  nLevel, char* szName, char *szClanName, unsigned int nClanID, MMatchUserGradeID nGrade, int duelTournamentGrade, int nCountry
							   ,int nR, int nG, int nB)
{
	...
	MBitmap *pBmpCountryFlag = NULL;

	char szCountryFlag[64];
	GetCountryFlagIconFileName(szCountryFlag, nCountry);
	pBmpCountryFlag = MBitmapManager::Get(szCountryFlag);

	ZLobbyPlayerListItem* pItem = new ZLobbyPlayerListItem(puid, MBitmapManager::Get(szFileName), nClanID, szLevel, szRefName, szClanName, state, nGrade, pBmpCountryFlag);
	pItem->SetColor(_color);

	MListBox::Add( pItem );
}
void ZPlayerListBox::AddPlayer(MUID& puid, MMatchObjectStageState state, int nLevel, char* szName, char* szClanName, unsigned int nClanID, bool isMaster, MMatchTeam nTeam, int duelTournamentGrade, int nCountry
							    ,int nR, int nG, int nB)
{
	if ( (int)strlen( szName) == 0) return;
	...
	ZStagePlayerListItem* pItem = new ZStagePlayerListItem(puid, pBitmap, nClanID, szRefName, szClanName, szLevel, gid, pBmpCountryFlag);
	pItem->SetColor(_color);
...
//			ZStagePlayerListItem* pItem = (ZStagePlayerListItem*) pWidget;
			if(m_nSelItem != -1) {
				...
				if(pItem) {
					...
					ZPostStageChat(ZGetGameClient()->GetPlayerUID(), ZGetGameClient()->GetStageUID(), temp, nRed, nGreen, nBlue);
				}
			}
		}
	}
