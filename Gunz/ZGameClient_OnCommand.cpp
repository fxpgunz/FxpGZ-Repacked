...
bool ZGameClient::OnCommand(MCommand* pCommand)
{
	...
	break;
		...
		case MC_KILL_THIS_PLAYER:
		{
									...
									pCommand->GetParameter(PlayerName, 0, MPT_STR, sizeof(PlayerName));
									...
									sprintf(szMsg, "^2%s has slained %s", PlayerName, TargetName);
									ZChatOutput(szMsg);
		}
		...
		case MC_MATCH_STAGE_RESPONSE_FORCED_ENTRY:			// 난입
			{
				...
				{
			ZApplication::GetGameInterface()->ShowMessage("");
				}
			}
			break;
