...
*/
	...
	if ( (ZGetGameClient()->AmIStageMaster() == true) && ( ZGetGameClient()->IsForcedEntry()))
	{
		if ( ZGetGameClient()->GetMatchStageSetting()->GetStageState() == STAGE_STATE_STANDBY)
		{
			...
			// 인터페이스관련
			ZApplication::GetGameInterface()->EnableWidget("StageSettingCaller", ZGetGameClient()->GetMatchStageSetting()->GetGameType() != MMATCH_GAMETYPE_SPY);	// 방설정 버튼
...
void ZStageInterface::ChangeStageGameSetting( const MSTAGE_SETTING_NODE* pSetting)
{
	...
	if ( (pSetting->nGameType == MMATCH_GAMETYPE_DEATHMATCH_SOLO) ||			// 데쓰매치 개인전이거나...
		 ...
		 (pSetting->nGameType == MMATCH_GAMETYPE_VAMPIRE) )							// 듀얼모드 이면...
	{
		// 맵 이름 배경 이미지 변환
		if ( pAniMapImg)
	...
	else if (pSetting->nGameType == MMATCH_GAMETYPE_DROPMAGIC)					// Magic Box
	{
		if (pAniMapImg)
...
#ifdef _QUEST
	if (ZGetGameTypeManager()->IsQuestDerived(pSetting->nGameType) || ZGetGameClient()->IsLadderGame() || pSetting->nGameType == MMATCH_GAMETYPE_DUELTOURNAMENT)	// Fixed for Duel Tournament.
	{
