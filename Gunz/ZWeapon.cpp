...
#define ROCKET_VELOCITY			2700.f	// 2004≥‚ 9ø˘ 21¿œ¿⁄ ¿Ã¿¸¿∫ 2500.f
#define ROCKET_SPLASH_RANGE		350.f	// ∑Œƒœ Ω∫«√∑°Ω√ π¸¿ß
#define ROCKET_MINIMUM_DAMAGE	.3f		// ∑Œƒœ πÃ¥œ∏ÿ µ•πÃ¡ˆ
#define ROCKET_KNOCKBACK_CONST	.5f		// ∑Œƒœø° ∆®∞‹¡Æ ≥™∞°¥¬ ªÛºˆ
...
#define ROCKET_LIFE			10.f		// 10√  µ⁄ø° ≈Õ¡¸
...
{
	// ¿⁄Ω≈¿« ¿Ã∆Â∆Æ ±◊∏Æ∞Ì..
	// ¡÷∫Ø ¿Ø¥÷µÈø°∞‘ µ•πÃ¡ˆ ¡÷¥¬ ∏ﬁΩ√¡ˆ ≥Ø∏Æ∞Ì..
	// ( ∏ﬁΩ√¡ˆ πﬁ¿∫∞˜ø°º≠ ¡÷∫Ø ¿Ø¥÷µÈ √º≈©«œ∞Ì.. 
	// ¡◊¿∫ ¿Ø¥÷¿Ã ¿÷¥Ÿ∏È πﬂªÁ¿Ø¥÷¿« ≈≥ºˆ∏¶ ø√∑¡¡÷±‚)
...
void ZWeaponItemKit::Render()
{
	if (m_bInit) {
		if (m_pVMesh->m_pMesh) {
			// π´±‚ ∆ƒ√˜∏¶ ƒ≥∏Ø≈Õø° ∫ŸæÓ¿÷¿ª∞ÊøÏ ¡∂∏Ì¿ª πﬁ∞Ì æ»∫ŸæÓ¿÷¿ª∞ÊøÏ ¡∂∏Ì¿ª æ»πﬁ¥¬ 2∞°¡ˆøÎµµ∑ŒªÁøÎ«œ±‚∂ßπÆ		
...
void ZWeaponItemKit::UpdateWorldItemFloorDrop(DWORD dwPickPassFlag)
{
	...
	if (bPicked && fabsf(Magnitude(rpi.PickPos - m_Position)) < 5.0f) {
		if (m_bWorldItemFloorDrop == false) {
			...
			m_fDeathTime = ZGetGame()->GetTime() + 2.0f;//≥◊∆Æø˜ ¿¸º€ Ω√∞£∞Ì∑¡ √÷¥Î 2.0√ µ⁄ø° ¿⁄µø º“∏Í
		}
	}
}
...
void ZWeaponItemKit::UpdatePos(float fElapsedTime, DWORD dwPickPassFlag)
{
	...
	if (bPicked && fabsf(Magnitude(pickpos - m_Position)) < fDist)
	{ // æ¥ˆ¿Ã≥™ ≥≠∞£¿ßø°º≠ ∂•ø° ∂≥æÓ¡¸(±‚∫ª ∂≥æÓ¡ˆ¥¬ Ω√∞£∫∏¥Ÿ ∏π¿Ã ∞…∏±∞ÊøÏ)
		m_Position = pickpos + normal;
		m_Velocity = GetReflectionVector(m_Velocity, normal);
		m_Velocity *= zpi.pObject ? 0.1f : 0.2f;	// ƒ≥∏Ø≈Õø° √Êµπ«œ∏È º”µµ∞° ∏π¿Ã ¡ÿ¥Ÿ
		m_Velocity *= 0.2f;
...
bool ZWeaponItemKit::Update(float fElapsedTime)
{
	...
	if (m_bWorldItemFloorDrop) // ø˘µÂ æ∆¿Ã≈€¿Ã ∂•ø° ∂≥æÓ¡ˆ∏È √≥∏Æ«œ¡ˆ æ ¥¬¥Ÿ.
		return true;

	//	if(m_bSendMsg) { // ø‹∫Œø°º≠ bDeath ∑Œ∏∏ ¡ˆøÓ¥Ÿ..
	...
	{	// ø˘µÂæ∆¿Ã≈€¿Ã ∂≥æÓ¡˙ Ω√∞£∞˙ ¿ßƒ°∏¶ πÃ∏Æ ∞ËªÍ»ƒ º≠πˆø° ∏ﬁºº¡ˆ∏¶ πÃ∏Æ ∫∏≥ª¡ÿ¥Ÿ....20090213 by kammir
		rvector vTempPos = m_Position;
		rvector vTempVelocity = m_Velocity;

		if (m_bSendMsg == false)
		{ // ∏ﬁºº¡ˆ∏¶ æ»∫∏≥¬¿ª∂ßø°∏∏ Ω««‡... «—π¯∏∏ ¿˚øÎµ≈∞⁄±›«—¥Ÿ.
			float fTick = 0.02f;
			for (int i = 0; i<500; i++)
			{ // 10√  ¡§µµ∏¶ πÃ∏Æ øπªÛ«œø© æ∆¿Ã≈€¿Ã ∂≥æÓ¡ˆ¥¬∞… »Æ¿Œ«—¥Ÿ.
				m_Velocity.z -= 500.f * fTick;
				UpdatePos(fTick, dwPickPassFlag);		// æ∆¿Ã≈€¿Ã ∞Ëº” ∂≥æÓ¡¯¥Ÿ.
				UpdateWorldItemFloorDrop(dwPickPassFlag);	// æ∆¿Ã≈€¿Ã ∂•ø° ∂≥æÓ¡≥¥¬¡ˆ »Æ¿Œ

				if (m_bWorldItemFloorDrop == true)
				{ // ∂•ø° ∫Œµ˙«˚¿∏∏È º≠πˆø° ∏ﬁºº¡ˆ∏¶ ≥Ø∏∞¥Ÿ.
					UpdatePost(dwPickPassFlag, fTick*i);
					m_bWorldItemFloorDrop = false;
					break;
				}
			}
			// ∏∏æ‡ 10√ ∞° ¡ˆ≥™µµ πŸ¥⁄ø° ∂≥æÓ¡ˆ¡ˆ æ æ“¿∏∏È æ∆¿Ã≈€¿ª ±◊≥… ¡◊ø©¡ÿ¥Ÿ.
			m_bSendMsg = true;
		}

		m_Position = vTempPos;			// ∏ﬁºº¡ˆ∏¶ ∫∏≥ª±‚¿ß«ÿ πÃ∏Æ ∞ËªÍ«— ∫Ø∞Êµ» ∞™¿ª ø¯∑°¥Î∑Œ µπ∑¡¡ÿ¥Ÿ.
		m_Velocity = vTempVelocity;
	}

	// ø˘µÂæ∆¿Ã≈€¿Ã ∂≥æÓ¡ˆ¥¬∞… »≠∏Èø° ∫∏ø©¡˙ºˆ¿÷∞‘ ∞ËªÍ«ÿ¡ÿ¥Ÿ.
	m_Velocity.z -= 500.f * fElapsedTime;		// Ω√∞£ø° µ˚∏• ∂•ø° ∂≥æÓ¡ˆ¥¬ º”µµ
	UpdatePos(fElapsedTime, dwPickPassFlag); // º”µµøÕ ø˘µÂæ∆¿Ã≈€¿« ≥Ù¿Ã∏¶ Ω√∞£¥Á ∞ËªÍ«‘¿∏∑ŒΩ· æ∆∑°∑Œ ∂≥æÓ¡¯¥Ÿ.
	UpdateWorldItemFloorDrop(dwPickPassFlag);	// æ∆¿Ã≈€¿Ã ∂•ø° ∂≥æÓ¡≥¥¬¡ˆ »Æ¿Œ(∂•πŸ¥⁄«œ∞Ì ø˘µÂæ∆¿Ã≈€¿« ≥Ù¿Ã∞° 5πÃ∏∏¿Ã∏È √≥∏Æ)
...
void ZWeaponGrenade::Create(RMesh* pMesh, rvector &pos, rvector &velocity, ZObject* pOwner) {
	...
	m_nSoundCount = rand() % 2 + 2;	// 2~ 3π¯ º“∏Æ∏¶ ≥Ω¥Ÿ
}


#define GRENADE_LIFE			2.f		// 3√  µ⁄ø° ≈Õ¡¸
...
bool ZWeaponGrenade::Update(float fElapsedTime)
{
...

	{
			...
			*/
		{
			m_Position = pickpos + normal;
			m_Velocity = GetReflectionVector(m_Velocity, normal);
			m_Velocity *= zpi.pObject ? 0.4f : 0.8f;		// ƒ≥∏Ø≈Õø° √Êµπ«œ∏È º”µµ∞° ∏π¿Ã ¡ÿ¥Ÿ
...
#define FLASHBANG_LIFE	3	//<<- TODO : π´±‚¿« º”º∫ø°º≠ πﬁæ∆ø¿±‚
#define FLASHBANG_DISTANCE	2000		// 20πÃ≈Õ ±Ó¡ˆ øµ«‚¿ª ¡‹
...
void ZWeaponFlashBang::Explosion()
{
	...
	if (dist > FLASHBANG_DISTANCE)	// ∞≈∏Æ π€ø°º≠¥¬ øµ«‚¿ª ¡÷¡ˆ ∏¯«‘
	{
		return;
	}

	if (ZGetGame()->m_pMyCharacter->IsDie())	// ø…¿˙πˆ ∏µÂø°º≠¥¬ øµ«‚¿ª πﬁ¡ˆ æ ¿Ω
	{
		return;
	}
...
#define MAGIC_WEAPON_LIFE			10.f		// 10√  µ⁄ø° ≈Õ¡¸
...
#define ANGULAR_VELOCITY	0.01f		// ∞¢º”µµ : √ ¥Á »∏¿¸∞°¥…«— ∞¢¿« ≈©±‚. (¥‹¿ß:∂Ûµæ»)
...
#define MAGIC_KNOCKBACK_CONST   .3f	
	...
	if (type == WMET_OBJECT) {// object ø° ∫Œµ˙»˜¥¬ ∞ÊøÏ...
		float fScale = m_fMagicScale;

		// ¿Ã∆Â∆Æ¥¬ Ω∫ƒ…¿œ∏µ«œ∏È ¡Ÿ¿Œ¥Ÿ.
...
bool ZWeaponTrap::Create(RMesh* pMesh, rvector& pos, rvector& velocity, int nItemId, ZObject* pOwner)
{
	...
	// º≠πˆø° ∆Æ∑¶¿ª ¥¯¡¯¥Ÿ¥¬ ∞Õ¿ª ∫∏≥Ω¥Ÿ. º≠πˆ¥¬ π∫∞° ¥¯¡Æ¡≥¥Ÿ¥¬ ∞Õ∏∏ ±‚æÔ«ÿµŒæ˙¥Ÿ∞° 
	// ¿Ã»ƒø° ∆Æ∑¶¿« πﬂµø ≈Î∫∏∏¶ ¥ŸΩ√ πﬁ¿ª∂ß æ∆ æ∆±Ó ¥¯¡Æ¡¯ æ÷∞° æÓµø°º≠ πﬂµøµ∆±∏≥™ «œ∞Ì √º≈©«œ∏È µ»¥Ÿ.
	// æÓ∂≤ ∆Æ∑¶¿Ã ¥¯¡Æ¡≥¿∏≥™ æ∆¡˜ πﬂµøµ«¡ˆ æ ¿∫ ªÛ≈¬¿œ ∂ß ≥≠¿‘«— ¿Ø¿˙ø°∞‘¥¬ ¿Ã πﬂµøµ» ∆Æ∑¶ø° ¥Î«ÿº≠ º≠πˆ∞° ∫∞µµ∑Œ ∏ﬁΩ√¡ˆ∏¶ ∫∏≥ª¡ÿ¥Ÿ
...
#define TRAP_LANDING_TIME 0.7f		// ∂•ø° ¥Í¿∫ »ƒ ¿Ã Ω√∞£µøæ» ∞°∏∏»˜ ±‚¥Ÿ∏∞¥Ÿ
...
bool ZWeaponTrap::Update(float fElapsedTime)
{
...

	{
		...
		if (bPicked && fabsf(Magnitude(pickpos - m_Position))<fDist)
		{
			...
			// ¡ˆ∏Èø° ¥Íæ“¿ª ∂ß ¿œ¡§Ω√∞£ »ƒ ∆¯πﬂ (∫Æ¿Ã≥™ √µ¿Â √Êµπ¿∫ ∆–Ω∫)
...
void ZWeaponTrap::Activate()
{
	...
	// º≠πˆø° πﬂµøµ» ∆Æ∑¶¿ª µÓ∑œ«ÿº≠ ¿Ã»ƒ ≥≠¿‘«œ¥¬ ¿Ø¿˙∞° ø˘µÂø° º≥ƒ°µ» ∆Æ∑¶µÈ¿ª æÀ ºˆ ¿÷µµ∑œ «œ¿⁄
	if (m_uidOwner == ZGetMyUID())
...
{
	0, 0.01, 0.03, 0.06, 0.1, 0.15, 0.21, 0.28, 0.36, 0.45, 0.55, 0.66
};
...
#define STUN_GRENADE_LIFE 0.15f	// √ππ¯¬∞ √Êµπ »ƒ 0.7√ »ƒø° ≈Õ¡ÅE
...
bool ZWeaponStunGrenade::Update(float fElapsedTime)
{
	...
	if (m_bCollided &&
		ZGetGame()->GetTime() - m_fStartTime > STUN_GRENADE_LIFE) {
		Explosion();
		if (Z_VIDEO_DYNAMICLIGHT)
			ZGetStencilLight()->AddLightSource(m_Position, 3.0f, 1300);
		return false;
...
void ZWeaponPortal::Explosion()
{
	...
	//ZGetSoundEngine()->PlaySound("fx_woosh",v);
