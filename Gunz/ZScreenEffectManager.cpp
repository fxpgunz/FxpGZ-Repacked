...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("%s : %f \n", x,f_time ); }
	...
	m_pWeaponIcons[MWT_TRAP_SPY] = NULL;
...
ZScreenEffect* ZScreenEffectManager::GetCurrWeaponImage()
{
	...
	if ((m_WeaponType == MWT_TRAP || m_WeaponType == MWT_TRAP_SPY) && m_SelectItemDesc)
	{
...
int ZScreenEffectManager::DrawResetGauges()
{
	...
	*/
	color = D3DCOLOR_ARGB(255, 230, 20, 20);
	...
	color = D3DCOLOR_ARGB(255, 30, 60, 255);
...
void ZScreenEffectManager::DrawGauges()
{
	...
	// ap
	color = D3DCOLOR_ARGB(255, 0, 140, 230); //Cloud: NewHP
	//color = D3DCOLOR_ARGB(255, 255, 255, 255);
	DrawGauge(80.f / 800.f, 47.f / 600.f, min(1.f, m_fGaugeAP) * 150.f / 800.f, 13.f / 600.f, 0.f, color);
	...
	DrawGauge(70.f / 800.f + x, 25.f / 600.f, w, 15.f / 600.f, 0.f, color);
	}
	...
	DrawGauge(70.f / 800.f + x, 45.f / 600.f, w, 15.f / 600.f, 0.f, color);
	}*/
