...
#define END_(x) { _end_time = timeGetTime(); float f_time = (_end_time - _begin_time) / 1000.f; mlog("\n-------------------> %s : %f \n\n", x,f_time ); }
	...
	ZLoadingProgress soundLoading("",pLoadingProgress,.12f);
	BEGIN_;
#ifdef _BIRDSOUND
	...
	ZLoadingProgress giLoading("",pLoadingProgress,.35f);
	...
	ZLoadingProgress meshLoading("",pLoadingProgress,.41f);
	BEGIN_;
	...
	ZLoadingProgress etcLoading("",pLoadingProgress,.02f);
	BEGIN_;
