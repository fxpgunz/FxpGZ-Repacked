...
bool ZCharacterItem::Confirm(MMatchCharItemParts parts, MMatchItemDesc* pDesc)
{
	...
	case MMCIP_MELEE:
		{
			...
			if( (strstr(ZGetGameClient()->GetStageName(), "[ns]")) || (strstr(ZGetGameClient()->GetStageName(), "[NS]")) )
			{
	...
	case MMCIP_SECONDARY:
	{
		...
		if ((strstr(ZGetGameClient()->GetStageName(), "[ng]")) || (strstr(ZGetGameClient()->GetStageName(), "[NG]")))
		{
