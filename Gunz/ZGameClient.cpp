...
int ZGameClient::OnResponseMatchLogin(const MUID& uidServer, int nResult, const char* szServerName, const MMatchServerMode nServerMode,
	const char* szAccountID, const MMatchUserGradeID nUGradeID, const MMatchPremiumGradeID nPGradeID, int nCountryFlag, int nCoins, int nECoins,
									  const MUID& uidPlayer, bool bEnabledSurvivalMode, bool bEnabledDuelTournament, unsigned char* pbyGuidReqMsg)
{
	int nRet = MMatchClient::OnResponseMatchLogin(uidServer, nResult, szServerName, nServerMode,
		szAccountID, nUGradeID, nPGradeID, nCountryFlag, nCoins, nECoins, uidPlayer, bEnabledSurvivalMode, bEnabledDuelTournament, pbyGuidReqMsg);

	ZGetMyInfo()->InitAccountInfo(szAccountID, nUGradeID, nPGradeID, nCountryFlag, nCoins, nECoins);
			...
			for(int i=0; i<nCount; i++){
				...
				if (pCache->CheckFlag(MTD_PlayerFlags_AdminHide) == false) {
					pList->AddPlayer(pCache->GetUID(),MOSS_NONREADY,pCache->GetLevel(),
						pCache->GetName(), pCache->GetClanName(), pCache->GetCLID(), false, MMT_ALL, pCache->GetDTGrade(), /*pCache->GetCountryFlag()*/
...
void ZGameClient::OnChannelChat(const MUID& uidChannel, char* szName, char* szChat, int nGrade, int nR, int nG, int nB)
{
	...
	strftime(szTimeStamp, 30, "[%H:%M:%S] : ", timeinfo);

	switch (nGrade) 
	{
	case 255:
		wsprintf(szText, "^1[AD]^9%s %s : %s", szTimeStamp, szName, szChat); break;
	case 254:
		wsprintf(szText, "^2[DV]^9%s %s : %s", szTimeStamp, szName, szChat); break;
	case 252:
		wsprintf(szText, "^4[PO]^9%s %s : %s", szTimeStamp, szName, szChat); break;
	case 0:
		wsprintf(szText, "^9%s %s : %s", szTimeStamp, szName, szChat); break;
	}
		...
		if( pNode ) 
		{
			...
			if ((pNode->nPlayerFlags & MTD_PlayerFlags_AdminHide) == true) {
				...
				pPlayerListBox->AddPlayer(pNode->uidPlayer, state, pNode->nLevel, pNode->szName, pNode->szClanName, pNode->nCLID, 
					(MMatchUserGradeID)pNode->nGrade, pNode->nDTLastWeekGrade, /*pNode->nCountryFlag,*/ pNode->nChannelRed, pNode->nChannelGreen, pNode->nChannelBlue);
			}
		}
	}
	if(vecClanID.size() > 0)
...
void ZGameClient::OnAdminWall(const char* szMsg, const char* szCharName)
{
	...
	if (ZApplication::GetGameInterface()->GetState() != GUNZ_GAME)
	{
		//sprintf(szTitle, "Anuncio Global");
		bShowMe = false;
		sprintf(szText, "^2[STAFF] ^1%s : ^4%s", szCharName, szMsg);
	...
	{
		bShowMe = true;
		//sprintf(szTitle, "^1[STAFF] ^2%s :", szCharName);
		//sprintf(szText, "^9Message : ^4%s", szMsg);
		sprintf(szText, "^2[STAFF] ^1%s : ^4%s", szCharName, szMsg);
...
void ZGameClient::OnGameLevelDown(const MUID& uidChar)
{
	if (ZGetGame())
	{
		...
		if (pCharacter) {
			...
			ZChatOutput(MCOLOR(ZCOLOR_GAME_INFO), temp);
			ZGetSoundEngine()->PlaySound("nar/NARDOWN", pCharacter->GetPosition()); //Levelup sound 
		}
	}
}
