	...
	case MWM_ACTIONPRESSED:
		if ( !ZGetGame()->IsReservedSuicide())		// 자살 예정인 경우 대쉬를 할수없게 막는다
		{
			...
			case ZACTION_DANCE:
				{
					...
					{
						ZMyItemNode* pItemNode = ZGetMyInfo()->GetItemList()->GetEquipedItem(MMatchCharItemParts(MMCIP_DANCE));
						if(!pItemNode) mtype = ZC_SPMOTION_DANCE1;
						else
						switch(pItemNode->GetItemID())
						{
						case 600406:
							{
								mtype = ZC_SPMOTION_DANCE1;
								break;
							}
						case 600407:
							{
								mtype = ZC_SPMOTION_DANCE2;
								break;
							}	
						case 600408:
							{
								mtype = ZC_SPMOTION_DANCE3;
								break;
							}
						case 600409:
							{
								mtype = ZC_SPMOTION_DANCE4;
								break;
							}
						case 600410:
							{
								mtype = ZC_SPMOTION_DANCE5;
								break;
							}
							default:
							{
			...
			case ZACTION_RECORD:
				...
				return true;
/*case ZACTION_MOVING_PICTURE: // Fix F10 Bug More Later...
				{	// 동영상 캡쳐...2008.10.02
	...
	case MWM_KEYDOWN:
		{
			...
			/*case 'M':
				...
				break;*/
			case 'M':
				if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_DEATHMATCH_SOLO ||
					...
					ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_PAINTBALL_SOLO ||
					ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_PAINTBALL_TEAM ||
					ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_MODE_STAFF ||
					ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_SNIPERMODE ||
					ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_GUNMODE ||
					ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_SPY ||
					ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_NINJA ||
					ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_NOKSTYLE ||
	...
	case MWM_MOUSEWHEEL:
		{
			...
			const ZMyCharaterStatusBitPacking & zStatus = ZGetGame()->m_pMyCharacter->m_statusFlags.Ref();
