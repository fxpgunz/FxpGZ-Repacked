...
bool ZShop::CheckTypeWithListFilter(int type, bool bEnchantItem)
{
	...
	else if (m_ListFilter == zshop_item_filter_dance) { if (type == MMIST_DANCE) return true; }
...
void ZShop::Serialize()
{
	...
	for (int i = 0; i<GetItemCount(); i++)
	{
		...
		if (MMatchItemDesc* pDesc = MGetMatchItemDescMgr()->GetItemDesc(m_vShopItem[i]->nItemID))
		{
			...
			/// Jorklenis2: <CashShop>
			if (pDesc->IsCashItem() && m_ShopMode != ZSHOP_CASH) continue;
			/// Jorklenis2: <EventShop>
			if (pDesc->IsEventItem() && m_ShopMode != ZSHOP_EVENT) continue;
			if (!pDesc->IsCashItem() && !pDesc->IsEventItem() && m_ShopMode != ZSHOP_NORMAL) continue;
		...
		else if (MQuestItemDesc* pDesc = GetQuestItemDescMgr().FindQItemDesc(m_vShopItem[i]->nItemID))
		{
			...
			if (m_ShopMode != ZSHOP_NORMAL) continue;
