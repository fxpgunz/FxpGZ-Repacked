...
bool ZGameInterface::OnCommand(MCommand* pCommand)
{
	...
	case MC_MATCH_RESPONSE_CHARACTER_ITEMLIST:
	{
		...
		/// Jorklenis2: <Cash y ECoins>
		pCommand->GetParameter(&nCoins, paramIndex++, MPT_INT);
		ZGetMyInfo()->SetCoins(nCoins);

		pCommand->GetParameter(&nECoins, paramIndex++, MPT_INT);
	...
	break;
	case MC_MATCH_RESPONSE_ACCOUNT_ITEMLIST: //Fix By Ryuzu
	{
		MCommandParameter* pParam = pCommand->GetParameter(0);
		void* pItemListBlob = pParam->GetPointer();
	...
	case MC_MATCH_RESPONSE_BUY_ITEM:
	{
		...
		else if ((nResult == MERR_TOO_EXPENSIVE_BOUNTY) || (nResult == MERR_TOO_MANY_ITEM) || (nResult == MERR_TOO_EXPENSIVE_CASH) || (nResult == MERR_TOO_EXPENSIVE_EVENT)) /// Jorklenis2: <CashShop y EventShop>
		{
			ZApplication::GetGameInterface()->ShowErrorMessage(nResult);
		}
		else
