...
/// Jorklenis2: <Support 16:9 aspect ratios>
...
void ZOptionInterface::InitInterfaceOption(void)
{
	...
	*/
	...
	if (pComboBox)
	{
		pComboBox->SetSelIndex(Z_EXTRA_DASH);
	}
	...
	//	ComboBox
	{
		...
		if(pWidget)
		{
			...
			/// Jorklenis2: <Corrección de error del iterador si la resolución no es compatible>
		...
		// µ¿¿µ»ó Ä¸ÃÄ 20081017... by kam
		pWidget = (MComboBox*)pResource->FindWidget("MovingPictureResolution");
		if(pWidget)	{
			pWidget->SetSelIndex(ZGetConfiguration()->GetMovingPicture()->iResolution); // µ¿¿µ»ó Ä¸ÃÄ ÇØ»óµµ ¼¼ÆÃ
		}
		// µ¿¿µ»ó Ä¸ÃÄ 20081028... by kam
		pWidget = (MComboBox*)pResource->FindWidget("MovingPictureFileSize");
		if(pWidget)	{
			pWidget->SetSelIndex(ZGetConfiguration()->GetMovingPicture()->iFileSize); // µ¿¿µ»ó Ä¸ÃÄ ÆÄÀÏÅ©±â(¿ë·®Á¦ÇÑ)
		}
	...
	//	Button
	{
		...
		if (pWidget)
		{
			pWidget->SetCheck(Z_EXTRA_HPAP_SCOREBOARD);
		}
	...
	{	
		...
		if (pComboBox)
		{
			pComboBox->SetSelIndex(Z_EXTRA_CAMERA);
			ZGetCamera()->RSetCameraDistance(Z_EXTRA_CAMERA);
		}
		...
		pComboBox = (MComboBox*)pResource->FindWidget("WeaponTracksOption");
		if (pComboBox != NULL) {
			if (Z_ETC_WEAPONTRACKS >= pComboBox->GetCount()) {
				Z_ETC_WEAPONTRACKS = 0;
			}
			pComboBox->SetSelIndex(Z_ETC_WEAPONTRACKS);
			RSetTracksSize(Z_ETC_WEAPONTRACKS);
		}
	}
...
bool ZOptionInterface::SaveInterfaceOption(void)
{
	...
	// »õ Å°¸ÊÈÄ ¸ðµç Å°»óÅÂ¸¦ release·Î ¼ÂÆÃ
	...
	*/
	...
	{ // µ¿¿µ»ó Ä¸ÃÄ 20081017... by kam
		...
		if(pWidget)	{
			...
			SetBandiCaptureConfig(iResolution);									// ¹ÝµðÄ¸Ã³ µ¿¿µ»ó ÇØ»óµµ¿¡ Àû¿ëÇÑ´Ù
		}
		pWidget = (MComboBox*)pResource->FindWidget("MovingPictureFileSize");
		int iFileSize = 0;
		if(pWidget)	{
			...
			SetBandiCaptureFileSize(iFileSize);								// ¹ÝµðÄ¸Ã³ µ¿¿µ»ó ÆÄÀÏÅ©±â ¼¼ÆÃ¿¡ Àû¿ëÇÑ´Ù
		}
	}
	{	// ¾ð¾î¼±ÅÃ
...
//#endif //_MULTILANGUAGE
	}
	{
		...
		if (pWidget)
		{
			Z_EXTRA_HPAP_SCOREBOARD = pWidget->GetCheck();
		}
		pWidget = (MButton*)pResource->FindWidget("DynamicLight");
		if(pWidget)
		{
	...
	{	// Etc
		...
		if (pComboBox)
		{
			Z_EXTRA_CAMERA = pComboBox->GetSelIndex();
			ZGetCamera()->RSetCameraDistance(Z_EXTRA_CAMERA);
		...
		pComboBox = (MComboBox*)pResource->FindWidget("WeaponTracksOption");
		if (pComboBox != NULL) {
			Z_ETC_WEAPONTRACKS = pComboBox->GetSelIndex();
			RSetTracksSize(Z_ETC_WEAPONTRACKS);
		}
	}
	{
...
void ZOptionInterface::AdjustMultipliedWidgetsManually()
{
	...
	if (pWidget) {
		...
		if ( pWidget)
		{
			...
			//jintriple3 ÇØ»óµµ¿¡ µû¸¥ ¼­¹ö ¼±ÅÃ Ã¢ À§Ä¡ Áß¾Ó¿¡¼­ ¾à°£ ¾Æ·¡·Î...
	...
	*/
	...
	if (pWidget) {	// ÀÌ ½æ³×ÀÏ ÀÌ¹ÌÁö´Â ¿ÍÀÌµå ÇØ»óµµ¿¡¼­µµ Á¤»ç°¢ÇüÀ¸·Î º¸ÀÌµµ·Ï Á¶Á¤
...
void ZOptionInterface::GetOldScreenResolution()
{
	...
	//jintriple3 ÇØ»óµµ º¯°æ ÈÄ ÇÁ¸® ºä¿¡¼­ Äµ½½ÇÏ¸é °ÔÀÓ ·ë ¸®½ºÆ®ÀÇ Å©±â°¡ ÀÌ»óÇØÁö´Â ¹ö±× ¼öÁ¤.
...
END_IMPLEMENT_LISTENER()
	...
	// ÀÌ ¿¡µðÆ®¹Ú½º¿¡¼­ ¸Þ½ÃÁö µÎ°³¸¦ Ã³¸®ÇØ¾ßÇÏ±â ¶§¹®¿¡ BEGIN_IMPLEMENT_LISTENER/END_IMPLEMENT_LISTENER ¸ÅÅ©·Î¸¦ ¾È½èÀ½
