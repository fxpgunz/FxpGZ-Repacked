...
void ZMyCharacter::OnUpdate(float fDelta)
{
	...
	//if (!ZGetGameClient()->GetMatchStageSetting()->IsModifierUsed(MMOD_BLOCK))
	{
		if (
#ifdef _DEBUG
			!m_bGuardTest &&
...
{
	if( (strstr(ZGetGameClient()->GetStageName(), "[nm]")) || (strstr(ZGetGameClient()->GetStageName(), "[NM]")) )
	{
		return;
	}
	else
...
void ZMyCharacter::OnMeleeGuardSuccess()
{
	...
	zStatus.m_bGuardCancel = true;
	//m_bCharged = true;
	if (m_pVMesh->m_SelectWeaponMotionType != eq_wd_spycase)
	{
		m_bCharged->Set_CheckCrc(true); //mmemory proxy
		m_fChargedFreeTime.Set_CheckCrc(ZGetGame()->GetTime() + COUNTER_CHARGED_TIME);
...
void ZMyCharacter::OnDamaged(ZObject* pAttacker, rvector srcPos, ZDAMAGETYPE damageType, MMatchWeaponType weaponType, float fDamage, float fPiercingRatio, int nMeleeType)
{
	...
	if ((ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_CLASSIC_SOLO) || (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_CLASSIC_TEAM))
		fDamage *= 3.f;
