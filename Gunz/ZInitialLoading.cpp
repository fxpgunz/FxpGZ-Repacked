	...
	float sx_, float sy_, float sw_, float sh_, bool bTipsVisible)
{
	...
	m_bTipsVisible = false/*bTipsVisible*/;
	m_szTips[0] = 0;
...
#ifdef _NOTICELOADING
	int nNotice = rand() % 3;
	switch (nNotice)
	{
	case 0:
		sprintf(m_szNotice, "The Cash Shop and Event Shop is now available within the game!");
		break;
	case 1:
		sprintf(m_szNotice, "Have fun!");
		break;
	case 2:
		sprintf(m_szNotice, "GunZ The Duel!");
		break;
	default:
		sprintf(m_szNotice, "");
		break;
...
void	ZInitialLoading::Draw(LOADING_SCENE_MODE mode_ /* = MODE_DEFAULT */, int destIndex_ /* = -1 */, bool bFlip_, bool fromThread_)
{
	...
	for (int i = 0; i < numDrawing; ++i)
	{
		...
		{
			/*DrawBar( (float)(RGetScreenWidth() * 0.1), (float)(RGetScreenHeight() * 0.9f),
			(float)(RGetScreenWidth() * 0.7500), (float)(RGetScreenHeight() * 0.006f),  mPercentage* 0.01f ); */
		...
		if (mpDC)
		{
			...
			mpDC->m_Font.DrawText(x, y, m_szNotice, 0xFF277CAF);
