	...
	if (szMsg[0] == '/')
	{
		if (strlen(szMsg) >= 2)
		{
			if (((szMsg[1] > 0) && (isspace(szMsg[1]))) == false)
			{
				...
				if ((ZGetMyInfo()->GetUGradeID() == MMUG_ADMIN) ||  // This flag is for admins and moderators (ranks 255, 254, 252) and ignores the Event Master (rank 251)
				...
				if ((ZGetMyInfo()->GetUGradeID() == MMUG_ADMIN) ||
					(ZGetMyInfo()->GetUGradeID() == MMUG_DEVELOPER) ||
					(ZGetMyInfo()->GetUGradeID() == MMUG_EVENTMASTER) ||  // This flag is for admins, moderators and event masters(ranks 255, 254, 252, 251)
					(ZGetMyInfo()->GetUGradeID() == MMUG_JORK))      // I did it because it's easy to manage the commands that the event master can or can't use.
				{
