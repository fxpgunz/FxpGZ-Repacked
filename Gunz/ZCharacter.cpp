...
static ZANIMATIONINFO g_AnimationInfoTableLower[ZC_STATE_LOWER_END] = {
	...
	{ "dance4", true, false, false, false },
	{ "dance5", true, false, false, false },
			...
			if (pWVMesh) {
				if (pWVMesh->GetMesh()) {
					...
					if (pItem && pItem->GetDesc())
						wtype = pItem->GetDesc()->m_nWeaponType.Ref();

					if (wtype == MWT_FLASH_BANG_SPY) {
						type = ZC_WEAPON_SP_FLASHBANG_SPY;
					}
					else if (wtype == MWT_SMOKE_GRENADE_SPY) {
						type = ZC_WEAPON_SP_SMOKE_SPY;
					}
					else if (wtype == MWT_TRAP_SPY) {
						type = ZC_WEAPON_SP_TRAP_SPY;
					}
					else if (wtype == MWT_STUN_GRENADE_SPY) {
						type = ZC_WEAPON_SP_STUN_SPY;
					}
					else if (strncmp(pWVMesh->GetMesh()->GetName(), "flashbang", 9) == 0) {
...
void ZCharacter::OnDraw()
{
	...
	//if (ZGetGame()->GetMatch()->GetMatchType() == MMATCH_GAMETYPE_INFECTED && uStatus.m_bCommander && this == m_pMyCharacter)
	//{
	//	if (!ZGetGame()->GetWorld()->IsFogVisible())
	//	{
	//		ZGetGame()->GetWorld()->SetFog(true);
	//		ZGetGame()->GetWorld()->m_dwFogColor
	//	}
	//}
...
#define VANISH_TIME		2.f		// 이 시간동안 투명해진다
	...
	m_pVMesh->Render(uStatus.m_bIsLowModel, false, IsDancing);
...
#define ELSE_IF_Lower_ENUM(a)	else if(a==m_AniState_Lower.Ref())	{ AddTextEnum(m_AniState_Lower.Ref(),a); }
	...
	IF_Upper_ENUM(ZC_STATE_UPPER_NONE)
		...
		*/
		...
		ELSE_IF_Lower_ENUM(ZC_STATE_DANCE4)
		ELSE_IF_Lower_ENUM(ZC_STATE_DANCE5)
...
void ZCharacter::InitProperties()
{
	...
	{
		m_Property.fMaxHP.Set_CheckCrc(950);
		m_fPreMaxHP = 950;
		m_Property.fMaxAP.Set_CheckCrc(950);
		m_fPreMaxAP = 950;
	}
	// MagicBox
