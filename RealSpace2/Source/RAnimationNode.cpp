...
D3DXQUATERNION RAnimationNode::GetRotValue(int frame)
{
	...
	s = (m_quat[p2+1].frame - m_quat[p2].frame );

	if (s != 0)	d = (float)(frame - m_quat[p2].frame) / (float)s;

	D3DXQuaternionSlerp(&rq,&m_quat[p2],&m_quat[p2+1],d);
...
D3DXVECTOR3 RAnimationNode::GetPosValue(int frame)
{
	...
	int s = (m_pos[p2+1].frame - m_pos[p2].frame );//tick

	float d = 1.f;

	if (s!=0)	d = (float)(frame%s)/(float)s;

	v = m_pos[p2] - m_pos[p2+1];
...
int	RAnimationNode::GetVecValue(int frame,D3DXVECTOR3* pVecTable)
{
	...
	int   s = m_vertex_frame[j2+1] - m_vertex_frame[j2];

	float d = 1;

	if (s != 0)	d = (frame - m_vertex_frame[j2] )/(float)s;

	D3DXVECTOR3* v1 = m_vertex[j2];
	D3DXVECTOR3* v2 = m_vertex[j2+1];
