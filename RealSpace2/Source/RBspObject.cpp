...
void RBspObject::ClearLightmaps()
{
	...
	// ¶óÀÌÆ®¸ÊÀÌ ¿©·¯ÀåÀÌ¾ú´Ù¸é Æú¸®°ï¿¡ »õ°ÜÁø ¹øÈ£µµ ¾ø¾Ú
	if( m_nLightmap )
	{
...
bool RBspObject::DrawTNT(RSBspNode *pNode,int nMaterial)
{
	...
	if(pNode->nPolygon)
	{
		...
		if(nCount)
		{
			...
			/*
			// ¹æ¹ý 2, ÇÑ³ëµå¾ÈÀÇ °°Àº material À» ¾²´Â ³Ñ³¢¸® ¹öÅØ½ºÀÇ min,max Ã£¾Æ¼­ Âï´Â´Ù
			int index=pNode->pIndicesOffsets[nMaterial];
...
// bsp ·Î Ãâ·ÂÇÏ´Â ¹æ¹ýÀÎµ¥, ¼Óµµ ¹®Á¦¶§¹®¿¡ ºÀÀÎ !
/*
...
//	if(m_ppLightmapTextures) return true;	// ÀÌ¹Ì ·ÎµåµÇ¾îÀÖ´Ù
	...
	int i;
//	int *pLightmapInfo=new int[m_nConvexPolygon];		// lightmap ¿¡ Áßº¹ÀÌ Á¦°ÅµÈ ÀÎµ¦½º.
...
//	mlog("RBspObject::OpenLightmap : nSourcePolygon = %d ,nNodeCount = %d\n",nSourcePolygon,nNodeCount);
	...
	for(i=0;i<m_nLightmap;i++)
	{
		...
		/// Jorklenis2: <Corrección de (lightmap texture »ý¼º ½ÇÆÐ %s \n)>
		if (hr != D3D_OK) mlog("lightmap texture failed to create: %s \n", DXGetErrorString9(hr));
		delete bmpmemory;
		//		delete memory;
	}
...
// ¿©·¯°³ °ãÃÄ ÀÖÀ»°æ¿ì¿¡´Â °¡Àå °¡±î¿î node ¸¦ Ã£¾Æ¾ß ÇÑ´Ù....
		...
		for(int i=0;i<pNode->nPolygon;i++)
		{
				...
				IsIntersect(g_PickOrigin,g_PickDir,*pVer->Coord(),*(pVer+2)->Coord(),*(pVer+1)->Coord(),&fDist))
			{
				...
				*g_pPickOutPathNode=pPathNode;
				...
				return true;  // ¹Ù·Î¸®ÅÏÇÏ¸é¾ÈµÇ°í.. ¸¶Àú ´Ù Ã£¾ÆºÁ¾ß ÇÑ´Ù .. ÀÏ´Ü ¾²´ÂÄÚµå°¡ ¾Æ´Ï¹Ç·Î »ý·« -_-;
			}
		}
	}
	else
...
bool RBspObject::GeneratePathData(const char *filename,float fAngle,float fToler)
{
	...
	{
		if(sin(fAngle)<fabs(m_pConvexPolygons[i].plane.c) && (m_pConvexPolygons[i].dwFlags & RM_FLAG_NOTWALKABLE)==0 )	// normal ÀÇ z °ª
		{
					...
					for(k=0;k<lightmapsize+1;k++)		// u
					{
						...
						/*
						{
							for(RMapObjectList::iterator i=m_ObjectList.begin();i!=m_ObjectList.end();i++)
							{
								...
								// ¹Ù¿îµù¹Ú½º°¡ ¸ðµ¨ÁÂÇ¥°è·Î µÇ¾îÀÖÀ¸¹Ç·Î ºûÀÇ ¹æÇâÀ» untransform ÇÑ´Ù.
								rmatrix inv;	
								float det;
...
//								rvector target=position*inv;
									...
									poi->pVisualMesh->Pick(plight->Position,dirorigin,&vOut,&t))
								{
									...
									// ÇöÀç À§Ä¡¸¦ ¸®ÅÏÇÏ´Â °ªÀÌ Á» ÀÌ»óÇØ¼­ ÁÖ¼®Ã³¸®ÇØµÒ.
	...
	if(pNode->nPolygon)
	{
		...
		if(nCount)
		{
			...
			for(i=0;i<nCount;i++)
			{
				...
				// ±¤¿ø µÞ¸é¿¡ ÀÖ´Â°ÍµéÀº (°©ÀÚ±â »ç¶óÁö¸é ÀÌ»óÇÏ¹Ç·Î) »¡¸® »ç¶óÁö°Ô ÇÑ´Ù
