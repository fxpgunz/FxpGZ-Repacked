...
void MBMatchServer::OnStageJoin(const MUID& uidChar, const MUID& uidStage)
{
	...
	if ((IsAdminGrade(pObj) == false) && pObj->GetAccountInfo()->m_nUGrade != MMUG_ADMIN && pObj->GetAccountInfo()->m_nUGrade != MMUG_JORK && pObj->GetAccountInfo()->m_nUGrade != MMUG_DEVELOPER && pStage->IsPrivate())
	{
...
int MBMatchServer::ValidateStageJoin(const MUID& uidPlayer, const MUID& uidStage)
{
	...
	{
		// PW
		// If the stage is a clan don't allow anyone to join.
		// Also checks if they're reconnecting to a stag
