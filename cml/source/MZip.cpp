...
// ¿©·¯ °¡Áö Àû¿ëÇÏ±â..Áö±ÝÀº µ¥ÀÌÅÍ Åä±Û¸µÀÌ µÊ,,, ¾ÏÈ£·Ö ¾Ë°í¸®µë Àû¿ë...
...
bool MZip::Initialize(FILE* fp, unsigned long ReadMode)
{
	...
	// zip , mrs1 ,mrs2 ¸¦ ÀÐ¾î¾ßÇÑ´Ù..
	// publish ¸é,,, zip À» ÀÐÁö ¸øÇÏµµ·Ï ¿É¼Ç¿¡ ¸ðµå¸¦ µÐ´Ù.. 
	// ÆÄÀÏ¿¡¼­´Â ÀÐ±â ºÒ°¡ + zip °¡´É¿©ºÎ + mrs1 °¡´É¿©ºÎ..
	// zip À» ÀÐÀ» ÇÊ¿ä°¡ ÀÖ´Â°¡?
	...
	if (m_nZipMode >= ZMode_Mrs2)							// mrs2 ÀÌ»óºÎÅÍ µ¥ÀÌÅÍ º¹±¸..
		RecoveryChar((char*)&dh, sizeof(MZIPDIRHEADER));		// v2 ÀÌ»óÀÌ¸é..

	// µ¥ÀÌÅÍ¸¦ Á¶ÀÛÇÏ´Â °æ¿ì.... zip , mrs1 , mrs2 ¸ðµÎ¾Æ´Ò°æ¿ì...
	...
	if (m_nZipMode >= ZMode_Mrs2)
		RecoveryChar((char*)m_pDirData, dh.dirSize);//mrs ¶ó¸é º¯È¯..
...
bool MZip::ReadFile(int i, void* pBuffer, int nMaxSize)
{
	...
	fseek(m_fp, h.fnameLen + h.xtraLen, SEEK_CUR);// MRS ÀÇ°æ¿ì º¹±¸ ¾ÈÇØ ÁáÀ½..
...
static char _fileheaderReader[1024 * 16];// sizeof(fh) + fh.fnameLen + fh.xtraLen + fh.cmntLen ÀÇ Å©±â..´ëÃæ
...
bool MZip::UpgradeMrs(char* mrs_name) // Mrs To Mrs2
{
	...
	if (fp == NULL) {
		mlog("%s ÆÄÀÏÀÌ ÀÐ±â Àü¿ëÀÎÁö È®ÀÎÇÏ¼¼¿ä!~ \n", mrs_name);
		return false;
	}
...
// ¸ðµç ÇÏÀ§Æú´õÀÇ ¿øÇÏ´Â ÆÄÀÏµé °Ë»ö..
