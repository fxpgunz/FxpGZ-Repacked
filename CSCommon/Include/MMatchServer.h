...
protected:
	...
	typedef map<MUID, unsigned long int> BlockTimerDef;
	BlockTimerDef BlockTimer;
...
protected:
	...
	bool StageAdd(MMatchChannel* pChannel, const char* pszStageName, bool bPrivate, const char* pszStagePassword, MUID* pAllocUID, bool bIsAllowNullChannel = false, bool bisEvent = false, MMATCH_GAMETYPE Type = MMATCH_GAMETYPE_DEFAULT);
...
#ifdef _ROOMEVENT
	void OnCreateEvent(bool bPrivate, char* pszStagePassword, MMATCH_GAMETYPE Type, int Limit, bool nAntiLead, int reward, int Players);
#endif
...
protected:
	...
	void OnAdminServerHalt(const MUID& uidPlayer);
...
public:
	...
	void OnStaffAnnounce(const MUID& uid, const char* szChat);
#ifdef _STAFFCHAT
