...
public:
	...
	bool BuyBountyItem(const unsigned int nCID, const unsigned int nAID, int nItemID, int nItemCount, int nPrice, const DWORD dwRentHourPeriod, const bool bIsSpendableItem, unsigned long int* poutCIID, int nBuyMode); /// Jorklenis2: <nBuyMode>
	...
	bool InsertQuestGameLog( const char* pszStageName, 
		...
		const int nMasterCID, const int nPlayer1, 
		const int nPlayer2, 
		const int nPlayer3, 
		const int nPlayer4, 
		const int nPlayer5, 
	...
	bool Ban(const int nAID, const bool bBan);
	bool VipWall(const int nAID);
	bool AdminGiveCoins(const int nAID, const int nCoins);
	bool WonEvent(const int nAid, const int reward);
