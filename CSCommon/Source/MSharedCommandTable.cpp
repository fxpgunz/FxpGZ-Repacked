...
//			P(MPT_BLOB, "SKey")
		...
		/*C(MC_MATCH_RESPONSE_CASH_ITEM, "Match.ResponseCashItem", "Response Cash Item", MCDT_MACHINE2MACHINE)
			P(MPT_INT, "result")*/
		...
		C(MC_MATCH_LADDER_REQUEST_CHALLENGE, "Match.Ladder.Request.Challenge", "Request Challenge a Ladder", MCDT_MACHINE2MACHINE)
			...
			P(MPT_UINT, "AntiLead")		// 레더팀 멤버 이름들
	...
	if(nSharedType&MSCT_MATCHSERVER || nSharedType&MSCT_CLIENT || nSharedType&MSCT_AGENT) {
		...
		if (nSharedType&MSCT_MATCHSERVER || nSharedType&MSCT_CLIENT) {
			...
			C(MC_ADMIN_REQUEST_KICK_PLAYER, "Admin.RequestKickPlayer", "Request Kick Player", MCDT_MACHINE2MACHINE)
				P(MPT_STR, "uidTargetCharName");
#ifdef _RELOADCONFIG
			C(MC_MATCH_RC, "RequestReloadConfig", "Request Reload Config", MCDT_MACHINE2MACHINE);
#endif
...
//	CA("connect", "Net.Connect");
	...
	C(MC_MATCH_HIT_REPLY, "DMGGven", "DMGiven", MCDT_PEER2PEER)
		P(MPT_INT, "ID")
	...
	C(MC_ADMIN_STAGE_JOIN, "STAFF.STAGE.JOIN", "STAFF STAGE JOIN", MCDT_MACHINE2MACHINE | MCCT_NON_ENCRYPTED)
		P(MPT_UID, "uidPlayer")
		P(MPT_STR, "name")
		C(MC_STAFF_HELP, "Staff.Help", "Announce", MCDT_MACHINE2MACHINE | MCCT_NON_ENCRYPTED)
		P(MPT_STR, "Msg");
