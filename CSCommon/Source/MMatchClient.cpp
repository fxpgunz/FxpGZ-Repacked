...
bool MMatchClient::OnCommand(MCommand* pCommand)
{
	...
	case MC_MATCH_RESPONSE_LOGIN:
	{
		...
		pCommand->GetParameter(&nCoins, 7, MPT_INT);
		pCommand->GetParameter(&nECoins, 8, MPT_INT);
		...
		OnResponseMatchLogin(pCommand->GetSenderUID(), nResult, szServerName, MMatchServerMode(nServerMode),
			szAccountID, MMatchUserGradeID(nUGradeID), MMatchPremiumGradeID(nPGradeID), nCountryFlag, nCoins, nECoins,
