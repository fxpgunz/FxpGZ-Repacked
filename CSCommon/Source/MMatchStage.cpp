...
bool MMatchStage::Create(const MUID& uid, const char* pszName, bool bPrivate, const char* pszPassword, bool bIsAllowNullChannel,
	const MMATCH_GAMETYPE GameType, const bool bIsCheckTicket, const DWORD dwTicketItemID, bool bisEvent)
{
	...
	m_bPrivate = bPrivate;
	m_bIsEvent = bisEvent;
	m_bEventFinnished = false;
	m_bIsUseResourceCRC32CacheCheck = MGetServerConfig()->IsUseResourceCRC32CacheCheck();
	if (bisEvent == true)
	{
		m_StageSetting.SetGameType(GameType);
		ChangeRule(GameType);
		OnStartGame();
		ChangeState(STAGE_STATE_RUN);
		m_pRule->isevent = true;
...
void MMatchStage::Tick(unsigned long nClock)
{
	...
	if ((m_ObjUIDCaches.empty()) && (GetState() != STAGE_STATE_CLOSE) && m_bIsEvent != true)
	{
		ChangeState(STAGE_STATE_CLOSE);
	}
...
bool _GetUserGradeIDName(MMatchUserGradeID gid,char* sp_name)
{
	...
	{ 
		if(sp_name) {
			strcpy(sp_name,"Developer or (GM)");
		}
		return true; 
	}
		...
		if(sp_name) { 
			strcpy(sp_name,"Administrator or (Owner)");
		}
		return true; 
	}
...
bool MMatchStage::StartGame(const bool bIsUseResourceCRC32CacheCheck)
{
	...
	if (pMasterObj && IsAdminGrade(pMasterObj) == false && (nPlayer > m_StageSetting.GetMaxPlayers()))
	{
		char szMsg[256];
	...
	for (MUIDRefCache::iterator i = m_ObjUIDCaches.begin(); i != m_ObjUIDCaches.end(); i++)
	{
		...
		{
			if (IsAdminGrade(pObj) && pObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) || pObj->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER && pObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) || pObj->GetAccountInfo()->m_nUGrade == MMUG_JORK && pObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide))
	...
	if (pMasterObj && IsAdminGrade(pMasterObj) && pMasterObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) || pMasterObj && pMasterObj->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER && pMasterObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide) || pMasterObj && pMasterObj->GetAccountInfo()->m_nUGrade == MMUG_JORK && pMasterObj->CheckPlayerFlags(MTD_PlayerFlags_AdminHide))
		bResult = true;
...
void MMatchStage::OnFinishGame()
{
	...
	if (m_bIsEvent == true)
	{
		MMatchStage* pStage = MMatchServer::GetInstance()->FindStage(GetUID());
		if (pStage == NULL) return;
		pStage->SetPassword("EventGunZ");
		pStage->SetPrivate(true);
		if (!m_StageSetting.IsTeamPlay())
		{
			struct winner{ int kills; MUID Uid; winner() { Uid = MUID(0, 0); kills = 0; } };
			winner winners[3];
			for (MUIDRefCache::iterator i = m_ObjUIDCaches.begin(); i != m_ObjUIDCaches.end(); i++)
			{
				MMatchObject* pObj = (MMatchObject*)(*i).second;
				if (pObj)
				{
					int kills = pObj->GetKillCount();
					for (int i = 0; i < 3; i++)
					{
						if (kills >= winners[i].kills)
						{
							winners[i].kills = kills;
							winners[i].Uid = pObj->GetUID();
							break;
						}
					}
				}
			...
			for (int i = 0; i < 3; i++)
			{
				if (winners[i].kills >= 0 && winners[i].Uid != MUID(0, 0))
				{
					MMatchObject* pObj = MMatchServer::GetInstance()->GetObject(winners[i].Uid);
					if (pObj)
					{
						MMatchServer::GetInstance()->GetDBMgr()->WonEvent(pObj->GetAccountInfo()->m_nAID, floor((EventReward / (i + 1)) + 0.5));
					}
				}
			}
		}
		...
		{
			MMatchTeam nWinnerTeam = MMT_RED;
			if (m_Teams[MMT_BLUE].nScore > m_Teams[MMT_RED].nScore)
				nWinnerTeam = MMT_BLUE;
			else if (m_Teams[MMT_RED].nScore > m_Teams[MMT_BLUE].nScore)
				nWinnerTeam = MMT_RED;
			else if (m_Teams[MMT_BLUE].nTotalKills > m_Teams[MMT_RED].nTotalKills)
				nWinnerTeam = MMT_BLUE;
			else if (m_Teams[MMT_RED].nTotalKills > m_Teams[MMT_BLUE].nTotalKills)
				nWinnerTeam = MMT_RED;
			else
				nWinnerTeam = MMT_ALL;
			for (MUIDRefCache::iterator i = m_ObjUIDCaches.begin(); i != m_ObjUIDCaches.end(); i++)
			{
				MMatchObject* pObj = (MMatchObject*)(*i).second;
				if (pObj && pObj->GetKillCount() >= 8)
				{
					if (nWinnerTeam == MMT_ALL)
						MMatchServer::GetInstance()->GetDBMgr()->WonEvent(pObj->GetAccountInfo()->m_nAID, floor((EventReward / 2) + 0.5));
					else if (pObj->GetTeam() == nWinnerTeam)
						MMatchServer::GetInstance()->GetDBMgr()->WonEvent(pObj->GetAccountInfo()->m_nAID, floor((EventReward)+0.5));
				}
			}
		}
	}
