...
void MMatchItemDesc::DumpBinary(FILE* fp)
{
	...
	fwrite(&m_bIsEventItem, sizeof(bool), 1, fp);
...
void MMatchItemDesc::LoadBinary(FILE* fp)
{
	...
	fread(&m_bIsEventItem, sizeof(bool), 1, fp);
...
bool MMatchItemDescMgr::ParseItem(MXmlElement& element)
{
	...
	for (int i = 0; i < nAttrCount; i++)
	{
		...
		else if (!stricmp(szAttrName, MICTOK_TYPE))
		{
			...
			else if (!_stricmp(szAttrValue, MICTOK_TYPE_DANCE))		pNewDesc->m_nType.Set(MMIT_DANCE);
		...
		else if (!stricmp(szAttrName, MICTOK_SLOT))
		{
			...
			else if (!_stricmp(szAttrValue, MICTOK_SLOT_DANCE))	    pNewDesc->m_nSlot = MMIST_DANCE;
		...
		else if (!stricmp(szAttrName, "iscashitem"))
		{
