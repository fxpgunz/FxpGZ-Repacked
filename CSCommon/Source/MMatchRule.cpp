...
#include "../SpyModeSystem/MMatchSpyMode.h" //PathSpy_Jork
...
bool MMatchRule::OnCheckBattleTimeOut(unsigned int tmTimeSpend)	// true 리턴하툈E타임아퓖E
{
...
void MMatchRule::CheckOnGameEvent()
{
	if( MMATCH_ROUNDSTATE_PLAY == GetRoundState() ) {
		...
		if (!isevent && nInGamePlayer == 0) SetRoundState(MMATCH_ROUNDSTATE_EXIT);
	}	
}
