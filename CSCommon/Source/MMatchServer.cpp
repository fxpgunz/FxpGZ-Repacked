...
#define FILENAME_SHOP					"xml/shopnormal.xml"
#define FILENAME_SHOPCASH				"xml/shopcash.xml"
#define FILENAME_SHOPEVENT				"xml/shopevent.xml"
...
bool MMatchServer::LoadInitFile()
{
	...
	*/
	...
	{
		Log(LOG_PROG, "Read Shop Normal Item Failed\n");
		return false;
	}
	...
	{
		Log(LOG_PROG, "Read Shop Item Event Failed\n");
		return false;
	}
...
void MMatchServer::OnRun(void)
{
	...
	// Ryuzu : Automatic Ranking System
...
#ifdef _ROOMEVENT
	static unsigned long AutoEventRun = nGlobalClock;
	if (nGlobalClock - AutoEventRun > TIMEAUTOEVENT1)
	{
		LOG(LOG_PROG, "ROOM EVENT HAS STARTED! (1)");
		if (GetClientCount() > 120)
		{
			int numb = RandomNumber(0, 3);
			switch (numb)
			{
			case 1:
				numb = MMATCH_GAMETYPE_DEATHMATCH_TEAM;
				break;
			case 3:
				numb = MMATCH_GAMETYPE_BERSERKER;
				break;
			}
			OnCreateEvent(false, "", (MMATCH_GAMETYPE)numb, 25, true, 10, 30);
			OnCreateEvent(false, "", (MMATCH_GAMETYPE)numb, 25, false, 10, 30);
			char szMsg[250];
			sprintf(szMsg, "%s event has started.", (MGetGameTypeMgr()->IsTeamGame((MMATCH_GAMETYPE)numb) ? "Team" : "Solo"));
		...
		AutoEventRun = nGlobalClock;
	}
	static unsigned long AutoEventRun2 = nGlobalClock;
	if (nGlobalClock - AutoEventRun2 > TIMEAUTOEVENT2)
	{
		LOG(LOG_PROG, "ROOM EVENT HAS STARTED! (2)");
		if (GetClientCount() > 140)
		{
			int numb = RandomNumber(0, 3);
			switch (numb)
			{
			...
			OnCreateEvent(false, "", (MMATCH_GAMETYPE)numb, 50, true, 30, 30);
			OnCreateEvent(false, "", (MMATCH_GAMETYPE)numb, 50, false, 30, 30);
			char szMsg[250];
			sprintf(szMsg, "%s event has started.", (MGetGameTypeMgr()->IsTeamGame((MMATCH_GAMETYPE)numb) ? "Team" : "Solo"));
			MCommand* pCmd = MMatchServer::GetInstance()->CreateCommand(MC_MATCH_ANNOUNCE, MUID(0, 0));
			pCmd->AddParameter(new MCmdParamUInt(0));
...
#define MINTERVAL_GARBAGE_SESSION_PING	(7 * 60 * 1000)	// 7 min
	...
	if (nGlobalClock - CustomEventTimers[2] >= 600000)
	{
		for (BlockTimerDef::iterator i = BlockTimer.begin(); i != BlockTimer.end();)
		{
			if (nGlobalClock - i->second >= 600000)
			{
				i = BlockTimer.erase(i);
			}
			else i++;
		}
		CustomEventTimers[2] = nGlobalClock;
	}
#ifdef _RELOADCONFIG
	if (nGlobalClock - CustomEventTimers[3] >= _RELOADCONFIGTIME)
	{
		CustomEventTimers[3] = nGlobalClock;

		if (ReloadConfigs == true)
		{
			if (!MMatchFormula::Create())
			{
				LOG(LOG_PROG, "Open Formula Table FAILED\n");
			...
			unsigned long nItemChecksum = MGetMZFileChecksum(FILENAME_ITEM_DESC);
			SetItemFileChecksum(nItemChecksum);
			LOG(LOG_PROG, "Cash and Event Shop has been update.");
			ReloadConfigs = false;
		}
	}
#endif
...
void MMatchServer::OnStaffAnnounce(const MUID& uid, const char* szChat)
{
	MMatchObject* pObj = GetObject(uid);
	if (pObj == NULL) return;
...
bool MMatchServer::CheckItemXML()
{
		...
		itor != MGetMatchItemDescMgr()->end(); ++itor)
	{
		...
		if (pItemDesc->IsCashItem())	nIsEventItem = 1;
		else							nIsEventItem = 0;
		...
		fprintf(fp, "UPDATE Item SET TotalPoint=0, BountyPrice=0, Damage=%d, Delay=%d, Controllability=%d, Magazine=%d, ReloadTime=%d, SlugOutput=%d, Gadget=0, SF=0, FR=0,CR=0,PR=0,LR=0, BlendColor=0, ModelName='', MaxBullet=%d, LimitSpeed=%d, IsCashItem=%d, IsEventItem=%d,\n",
			nDamage, nDelay, nControl, nMagazine, nReloadTime, nSlugOutput, nMaxBullet,
			pItemDesc->m_nLimitSpeed.Ref(), nIsCashItem, nIsEventItem);
