...
// SQL ////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
TCHAR g_szDB_WonEvent[] = _T("{CALL spWonEvent (%d, %d)}");
...
// 이벤트 짱마크 부여
// 인자 GradeID, AID
TCHAR g_szDB_EVENT_JJANG_UPDATE[] = _T("UPDATE Account SET PGradeID=%d WHERE AID=%d");
// 케릭터의 퀘스트 아이템 정보 요청.
...
TCHAR g_szDB_VIP_WALL[] = _T("{CALL spUpdateWalls (%d)}");
...
bool MMatchDBMgr::GetAccountInfo(const unsigned long int nAID, MMatchAccountInfo* poutAccountInfo, const int nServerID)
{
	...
	poutAccountInfo->m_nCoins = rs.Field("Coins").AsLong();
	poutAccountInfo->m_nECoins = rs.Field("ECoins").AsLong();
	...
	poutAccountInfo->m_nWalls = rs.Field("Walls").AsInt();
...
bool MMatchDBMgr::BuyBountyItem(const unsigned int nCID, const unsigned int nAID, int nItemID, int nItemCount, int nPrice, const DWORD dwRentHourPeriod, const bool bIsSpendableItem, unsigned long int* poutCIID, int nBuyMode) /// Jorklenis2: <nBuyMode>
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		strSQL.Format(g_szDB_BUY_BOUNTY_ITEM, nCID, nAID, nItemID, nItemCount, nPrice, bIsSpendableItem, dwRentHourPeriod, nBuyMode); /// PlayGunZ: <nBuyMode>
		rs.Open(strSQL, CRecordset::forwardOnly, CRecordset::readOnly);
	}
	catch (CDBException* e) {
...
bool MMatchDBMgr::Ban(const int nAID, bool bBan)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
...
bool MMatchDBMgr::VipWall(const int nAID)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;
	...
	try {
		strSQL.Format(g_szDB_VIP_WALL, nAID);
		m_DB.ExecuteSQL(strSQL);
	}
	catch(CDBException* e){
		ExceptionHandler(strSQL, e);
...
bool MMatchDBMgr::WonEvent(const int nAid, const int reward)
{
	_STATUS_DB_START;
	if (!CheckOpen()) return false;	
	...
	try {		
		strSQL.Format(g_szDB_WonEvent, nAid, reward);
		m_DB.ExecuteSQL( strSQL );
	} 
	catch(CDBException* e) {		
		ExceptionHandler(strSQL, e);
