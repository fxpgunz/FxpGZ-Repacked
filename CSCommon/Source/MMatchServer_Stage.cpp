...
#include "../SpyModeSystem/MMatchRuleSpy.h"
...
bool MMatchServer::StageAdd(MMatchChannel* pChannel, const char* pszStageName, bool bPrivate, const char* pszStagePassword, MUID* pAllocUID, bool bIsAllowNullChannel, bool bisEvent, MMATCH_GAMETYPE Type)
{
	...
	if (!pStage->Create(uidStage, pszStageName, bPrivate, pszStagePassword, bIsAllowNullChannel, Type, bIsCheckTicket, dwTicketID, bisEvent)) {
		if (pChannel) {
...
#ifdef _ROOMEVENT
void MMatchServer::OnCreateEvent(bool bPrivate, char* pszStagePassword, MMATCH_GAMETYPE Type, int Limit, bool nAntiLead, int reward, int Players)
{
	MMatchChannel* pChannel = FindChannel(MCHANNEL_TYPE_PRESET, "Reward");
	if (pChannel == NULL) return;
	MUID uidStage;
	char pszStageName[250];
	sprintf(pszStageName, "%s | %s | %d %s | %d reward", (MGetGameTypeMgr()->IsTeamGame(Type) ? "Team" : "Solo"), (nAntiLead ? "Anti" : "Lead"), Limit, (MGetGameTypeMgr()->IsTeamGame(Type) ? "Rounds" : "Kills"), reward);
	if (!StageAdd(pChannel, pszStageName, bPrivate, pszStagePassword, &uidStage, false, true, Type))
	{
		return;
	}
	MMatchStage* pStage = FindStage(uidStage);
	pStage->ChangeRule(Type);
	pStage->SetAntiLead(nAntiLead);
	pStage->GetStageSetting()->SetRoundMax(Limit);
	pStage->GetStageSetting()->SetGameType(Type);
	pStage->GetStageSetting()->SetLimitTime(25 * 60 * 1000); //Update Time
	pStage->GetStageSetting()->SetMaxPlayers(Players);
	pStage->GetStageSetting()->SetMapIndex(RandomNumber(MMATCH_MAP_MANSION, MMATCH_MAP_CITADEL));
	if (MGetGameTypeMgr()->IsTeamGame(Type))
	{
		pStage->GetStageSetting()->SetAutoTeamBalancing(false);
	}
	pStage->EventReward = reward;
}
#endif
void MMatchServer::OnStageCreate(const MUID& uidChar, char* pszStageName, bool bPrivate, char* pszStagePassword)
{
	...
	if ((MGetServerConfig()->GetServerMode() == MSM_QUEST) && (pChannel->GetChannelType() == MCHANNEL_TYPE_CLAN)
...
void MMatchServer::OnStageStart(const MUID& uidPlayer, const MUID& uidStage, int nCountdown)
{
	...
	MMatchChannel* pChan = MMatchServer::GetInstance()->FindChannel(pStage->GetOwnerChannel());
	if (pChan == NULL) return;
	MMatchObject* pMasterObj = MMatchServer::GetInstance()->GetObject(uidPlayer);
	if (!pMasterObj) return;
	if (pStage->m_bIsEvent == false && strstr(pChan->GetName(), "Reward") && pMasterObj && !IsAdminGrade(pMasterObj) || pStage->m_bEventFinnished == true && pStage->m_bIsEvent == true)
	{
		char szMsg[256];
		sprintf(szMsg, "Start Game Failed Event Has Finnished.");
...
void MMatchServer::OnStageTeam(const MUID& uidPlayer, const MUID& uidStage, MMatchTeam nTeam)
{
	...
	if (pStage->GetStageType() == MST_LADDER && !IsAdminGrade(pChar) && !pChar->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER && !pChar->GetAccountInfo()->m_nUGrade == MMUG_JORK || pStage->m_bIsEvent && !IsAdminGrade(pChar) && !pChar->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER && !pChar->GetAccountInfo()->m_nUGrade == MMUG_JORK || pStage->GetStageType() == MST_PLAYERWARS && !IsAdminGrade(pChar) && !pChar->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER && !pChar->GetAccountInfo()->m_nUGrade == MMUG_JORK) return;
	if (pStage->GetStageSetting()->GetGameType() == MMATCH_GAMETYPE_DEATHMATCH_TEAM &&
		pChar->GetTeam() == nTeam)	{	// 3 Team DM.
		nTeam = MMT_RED;
	}
...
void MMatchServer::OnStageSetting(const MUID& uidPlayer, const MUID& uidStage, void* pStageBlob, int nStageCount)
{
	...
	// Force Room TypeGame MagicBox (no join)
/*	if ((strstr(pChannel->GetName(), "Free")))
	{
		if ((MGetGameTypeMgr()->IsCorrectGameType(MMATCH_GAMETYPE_DROPMAGIC)))
		{
			if (pNode->bForcedEntryEnabled == true) pNode->bForcedEntryEnabled = false;
			pNode->nLimitTime = STAGESETTING_LIMITTIME_UNLIMITED;

		}
		if (MGetGameTypeMgr()->IsCorrectGameType(MMATCH_GAMETYPE_DROPMAGIC) == true &&
			MGetGameTypeMgr()->IsCorrectGameType(MMATCH_GAMETYPE_DROPMAGIC) == false)
...
void MMatchServer::OnVoteCallVote(const MUID& uidPlayer, const char* pszDiscuss, const char* pszArg)
{
	...
	MMatchStage* pStage = FindStage(pObj->GetStageUID());
	if (pStage == NULL) return;
	if (pStage->m_bIsEvent == true) return;
	char szMsg[256];
	// 운영자가 같이 게임중이면 투표 불가능
	for (MUIDRefCache::iterator itor = pStage->GetObjBegin(); itor != pStage->GetObjEnd(); itor++) {
		MUID uidObj = (MUID)(*itor).first;
		MMatchObject* pPlayer = (MMatchObject*)GetObject(uidObj);
		if ((pPlayer) && (IsAdminGrade(pPlayer)) || (pPlayer) && pPlayer->GetAccountInfo()->m_nUGrade == MMUG_DEVELOPER || (pPlayer) && pPlayer->GetAccountInfo()->m_nUGrade == MMUG_JORK)
		{
