...
#ifdef _RELOADCONFIG
	if (!m_MatchDBMgr.GetLoginInfo(szUserID, &nAID, szDBPassword) || ReloadConfigs == true)
#else
	...
	const unsigned char* pbyGuidReqMsg)
{
	...
	/// Jorklenis2: <Cash y ECoins>
	pCmd->AddParameter(new MCommandParameterInt(nCoins));
...
MCommand* MMatchServer::CreateCmdMatchResponseLoginFailed(const MUID& uidComm, const int nResult)
{
	...
	pCmd->AddParameter(new MCommandParameterInt(0)); // FLAGS
